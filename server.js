...
io.on('connection', (socket) => {
    ...
    socket.on('requestRoll', (data) => {
        ...
        // 서버에서 난수 생성 (완전 공정!)
        const result = Math.floor(Math.random() * gameState.diceMax) + 1;
        ...
        console.log(`${userName}이(가) ${result} 굴림 (서버 생성) - (${gameState.rolledUsers.length}/${gameState.gamePlayers.length}명 완료)`);
