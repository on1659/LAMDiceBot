...
## 📅 2026-01-10 업데이트 (채팅 이모티콘 반응 시스템 추가)
...
## 📅 2026-01-09 업데이트 (주사위 게임 진행 상황 및 결과창 표시 개선)

### 주요 변경사항

#### 1. 주사위 진행 상황 실시간 갱신 개선
- **진행 상황 즉시 업데이트**
  - 주사위를 굴릴 때마다 `rollProgress` 이벤트를 전송하여 진행 상황을 실시간으로 갱신
  - "주사위를 굴리지 않은 사람" 목록이 정확하게 업데이트되도록 개선
  - 주사위를 굴린 후 즉시 남은 인원 수가 반영됨

#### 2. 하이/로우/니어 게임 애니메이션 조건 확장
- **기존 조건 유지**
  - 하이 게임: 지금까지 나온 값 중 최저값보다 작은 값 → 항상 애니메이션
  - 로우 게임: 지금까지 나온 값 중 최고값보다 큰 값 → 항상 애니메이션
  - 니어 게임: 타겟 숫자에 가장 가까운 값 → 항상 애니메이션

- **추가 조건 (확률적 애니메이션)**
  - 하이 게임: 두번째로 큰 값일 때 10% 확률, 세번째로 큰 값일 때 5% 확률로 애니메이션
  - 로우 게임: 두번째로 큰 값일 때 10% 확률, 세번째로 큰 값일 때 5% 확률로 애니메이션
  - 니어 게임: 두번째로 가까운 값일 때 10% 확률, 세번째로 가까운 값일 때 5% 확률로 애니메이션
  - 애니메이션이 나와도 항상 걸린 것은 아니라는 점을 명확히 함

#### 3. 게임 종료 타이밍 개선
- **애니메이션 완료 후 종료 메시지 표시**
  - 마지막 주사위 애니메이션이 완료된 후에 게임 종료 메시지가 표시되도록 대기 로직 추가
  - 애니메이션 완료 후 최소 0.3초 대기 (애니메이션 효과를 볼 수 있도록)
  - 애니메이션이 없는 경우 0.5초 대기 후 즉시 처리
  - 최대 5초 타임아웃으로 무한 대기 방지

#### 4. 준비한 사람 제외 로직 개선
- **방에 없는 사용자도 제외 가능**
  - 호스트가 준비한 사람을 제외할 때, 방에서 이미 나간 사용자도 준비 목록에서 제거 가능
  - 준비 상태로 설정할 때만 방에 있는지 확인
  - 준비 취소는 방에 없어도 가능하도록 수정
...
- `requestRoll` 핸들러에서 `rollProgress` 이벤트 전송 로직 추가
  - 주사위를 굴릴 때마다 진행 상황을 모든 클라이언트에 전송
- 하이/로우/니어 게임 애니메이션 조건 확장
  - 두번째/세번째 값에서 확률적 애니메이션 로직 추가
  - `Math.random()`을 사용하여 확률 계산
- `setUserReady` 핸들러 개선
  - 준비 취소 시 방에 없는 사용자도 처리 가능하도록 수정
  - 준비 상태로 설정할 때만 방에 있는지 확인

#### 클라이언트 측 (dice-game-multiplayer.html)
- `gameEnded` 이벤트 핸들러에 애니메이션 완료 대기 로직 추가
  - 마지막 굴리는 사람의 애니메이션이 완료될 때까지 대기
  - 애니메이션 완료 후 최소 0.3초 대기
- 결과창 표시 시 중복 방지 로직 강화
  - 전역 플래그 사용
  - DOM 확인 로직 추가
  - 전역 `waitInterval` 관리
- 게임 시작/종료 시 플래그 및 interval 정리
...
### 사용자 경험 개선
- 주사위를 굴릴 때마다 진행 상황이 즉시 업데이트되어 게임 진행 상황을 정확하게 파악 가능
- 애니메이션이 나와도 항상 걸린 것은 아니라는 점을 명확히 하여 게임의 긴장감 유지
- 게임 종료 메시지가 적절한 타이밍에 표시되어 사용자 경험 향상
- 결과창이 중복으로 표시되지 않아 깔끔한 UI 유지
- 방에서 나간 사용자도 준비 목록에서 제거 가능하여 관리 편의성 향상
...
## 📅 2025-12-21 업데이트 (호스트의 참여자 강퇴 기능 및 자동 게임 종료 로직 개선)

### 주요 변경사항

#### 1. 참여자 강퇴 기능 (Kick) 추가
- **호스트 전용 강퇴 기능**
  - 호스트가 접속자 목록에서 다른 사용자를 더블 클릭하여 방에서 제외 가능
  - 실수 방지를 위한 확인 창(confirm) 표시
  - 강퇴된 사용자에게는 사유 알림 및 자동 로비 이동 처리

#### 2. 게임 참여자 보호 및 종료 로직 개선
- **굴리지 않은 사람만 강퇴 가능**
  - 게임 진행 중 이미 주사위를 굴린 사람은 강퇴할 수 없도록 보호하여 게임의 공정성 유지
- **자동 게임 종료 조건 강화**
  - 참여자가 강퇴되거나 방을 나갈 때마다 남은 인원이 모두 주사위를 굴렸는지 즉시 확인
  - 모든 인원이 완료된 상태가 되면 즉시 게임 결과 정산 및 자동 종료

### 기술적 변경사항

#### 서버 측 (server.js)
- **공통 종료 체크 함수 도입**
  - `checkAndEndGame(gameState, room)` 함수를 통해 종료 조건을 일관되게 관리
- **이벤트 핸들러 통합**
  - `kickPlayer` 이벤트 추가 및 `leaveRoom` 이벤트 내 종료 체크 로직 통합
  - 비정상 종료(소켓 유실) 시에도 데이터 정합성을 유지하도록 리스트 정리 로직 강화

#### 클라이언트 측 (dice-game-multiplayer.html)
- **더블 클릭 이벤트 지원**
  - 호스트 UI에서만 작동하는 더블 클릭 리스너 및 시각적 피드백(포인터, 툴팁) 추가
- **강퇴 알림 처리**
  - `kicked` 이벤트를 통한 즉각적인 페이지 처리 및 알림창 구현

### UX 개선
- 비매너 또는 잠수 사용자를 방장이 직접 제재할 수 있어 원활한 게임 진행 가능
- 중도 퇴장자가 발생해도 게임이 멈추지 않고 즉시 결과가 도출되어 대기 시간 감소
...
## 📅 2025-12-09 업데이트 (개선사항 게시판 기능 추가)
...
- **Postgres 모듈 선택적 로드**
  - `pg` 모듈이 없어도 서버 실행 가능
  - `DATABASE_URL` 환경변수로 Postgres 연결
  - 서버 시작 시 테이블 자동 생성 (`suggestions` 테이블)
