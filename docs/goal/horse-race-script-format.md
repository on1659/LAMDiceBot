...
- 프레임률·`deltaTime` 편차가 위치 오차로 쌓인다 → 서버 순위와 클라 렌더가 어긋날 수
  있다. 과거 이걸 150ms 최소 결승 간격으로 막았고(`68cd1b6`), 지금은 그 규칙이 제거된
  대신 **`finishStun`**(상위 순위 말이 미도착이면 하위 말을 결승선 앞에서 자빠뜨려
  세움, `js/horse-race.js:4455`)이 순위 정합을 강제한다 — 정확하지만, 격차가 좁으면
  결승 직전에 말이 넘어지는 그림이 자주 나온다 (실측: 판의 약 67%에서 최소 1회 발동)
...
결과: **드리프트 0, seek 공짜, 단일 SSOT.** 순위 정합을 위한 안전장치(150ms 간격 ·
`finishStun`)가 **구조적으로 불필요**해진다 — 클라가 위치를 계산하지 않고 스크립트가
준 값을 읽기만 하므로 어긋날 여지가 없다.

다만 결승 분리는 **연출 규칙으로 남긴다**: 완주 간격이 너무 좁으면 누가 이겼는지
읽히지 않는다. 즉 판정 시각 최소 간격은 "동기화 안전마진"에서 **"가독성 규칙"**으로
성격이 바뀐다. 접전을 의도한 D3 포토피니시에서는 이 간격을 좁히고 📸 판독 연출로
가독성을 대신 확보한다.

**핵심은 결승선 분리와 레이스 중 접전이 서로 다른 층이라는 것이다.** 깔끔한 결승
분리(가독성)와 그 전까지의 엎치락뒤치락(L0 계약)은 충돌하지 않는다 — 오히려 L0가
중간 순위 변동을 보장할수록, 결승선에서의 분리가 "드라마 뒤의 명확한 결말"로 읽힌다.
...
| 순위 정합 안전장치 | `finishStun` 자빠짐 클램프 (판의 ~67%에서 발동) | 불필요 — 클라가 위치를 계산하지 않음 |
| 결승 최소 간격 | 없음 (150ms 규칙은 `3b96b1d`로 제거됨) | 가독성 규칙으로 재도입 (D3는 예외) |
