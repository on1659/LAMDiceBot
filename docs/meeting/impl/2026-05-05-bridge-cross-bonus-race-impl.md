...
| 5 | 정상 점프 칸수 | **+1** |
| 6 | 종료 조건 | 모든 user가 8칸 도달 또는 max 12 turn 도달 |
...
| 16 | **용어 — wave → 턴 (Turn)** | 한 번의 점프 사이클 = 1 turn. UI/코드 모두 "턴" 통일. 영문 변수명은 일관성 위해 **`wave` 유지** (코드는 internal, UI 라벨은 "턴 K/12") |
...
1. **server가 라운드 시작 시 8 col 보너스 row 결정** — 비밀, 매 라운드마다 새로 추첨
2. **turn k 시작** (k = 1...max 12):
...
3. **server 판정**:
   ...
   - **turn-end 시점**에 progress=8 도달자를 finishOrder에 push (도달 순). 같은 turn에 여러 명 도달 시 advance 큰 user 우선, 동률이면 pendingChoices 처리 순서로 tie-break (서버 deterministic)
4. **시각화**:
   - 각 user 시차 점프 (deterministic seed, turn 내 0.18s 간격)
   - 점프 호/거리 = advance 칸 수 비례 (+1=jumpHeight 46, +2=70, +3=95)
   - 점프 거리(col 단위) = advance만큼 전진 (+1=1col, +2=2col, +3=3col)
   - 8칸 도달자: turn 점프 끝난 후 `random 0~800ms delay` 시차로 finish slot 점프 (turn-end 후 0.5~1.3s 사이 finish 도착)
5. **다음 turn**: 도달 안 한 user만 진행. server 대기 시간 `BRIDGE_INTER_TURN_MS = 1800ms` (turn 시각 시간 + finish 시차 delay 충분 보장)
6. **종료 조건 검사 (1라운드 안)**:
   - 도달 안 한 user 0명 → 라운드 종료, 결과 표시
   - 도달 안 한 user 1명 → 그가 당첨자, 라운드 종료
   - 도달 안 한 user 2명+ + currentTurn < 12 → 다음 turn
   - 도달 안 한 user 2명+ + currentTurn >= 12 → **sudden death turn** (도달 안 한 user들끼리만 추가 turn 반복, bonusRows 매번 새로 추첨, 1명 남을 때까지 무한 loop). max sudden death = 6 안전장치 (6번 sudden death 후에도 동률이면 random tie-break)
...
- gameEnd 결과 overlay: "🎯 주문 받을 사람: **{꼴등이름}**" + 도착 순서 (1등~꼴등) + 각 user의 progress
...
gameState.bridgeCross = {
  ...
  bonusRows: [],            // length 8 (라운드 시작 시 추첨), 'top'|'bottom'
...
state.currentWave = 0;
state.maxWaves = 12;            // 서버 maxWaves 캐시 (sudden death 시 표시 분기용)
...
- `BRIDGE_MAX_WAVES = 12` (max turn 수, sudden death는 별도 카운터)
...
| `bridge-cross:waveResult` | `{wave, results: [{userName, choice, advance: 1\|2\|3, newProgress}], finishedThisWave: [...userName], isSuddenDeath: bool}` | **success bool 폐기**, advance/newProgress 신규. 클라가 모든 user를 추락 인지하지 않도록 success 필드 자체 삭제 (Codex 함정 §F-5) |
...
4. processWave:
   ...
   - bonusRow / bonusAmount 결정:
     * normal turn (1~12): bonusRows[wave-1], bonusAmounts[wave-1] 사용
     * sudden death turn (13+): 매번 새로 추첨 (`Math.random() < 0.5 ? 'top' : 'bottom'`,
       bonusAmount는 항상 +8 강제 — sudden death는 1명만 못 건너기 위해 보너스 받은 user 즉시 도달)
   - 각 choice를 결정된 bonusRow와 비교
   - advance = (match ? bonusAmount : 1)
   - userProgress[name] += advance, clamp 8
   - newProgress >= 8인 user를 finishOrder에 push:
...
idle/ready-wait → playing
  enter-bridge (모두 다리 진입 점프)
    ↓
  turn-wait (좌/우 선택 UI 활성, 도달 안 한 user만, 라벨 "턴 K/12" 또는 "🔥 sudden death")
    ↓ waveResult
...
- 다리 8 col. **world 폭 확장 + tileSize 유지** (시각 충돌 회피):
  - 기존: entrance/exit 거리 = `(6-1) * columnStep`, world ≈ 1024px
  - 신규: entrance/exit 거리 = `(8-1) * columnStep`, columnStep 동일 유지 → world 폭 ≈ +40% 확장
  - 캔버스 viewport 1024×683 그대로. **`Camera`/`CameraDirector`가 자동으로 follow + zoom-out 처리** (기존 인프라 `js/bridge-cross.js:1444~1498`)
  - `Bridge` 생성자 (`js/bridge-cross.js:1336-1351`)는 entrance/exit를 입력으로 받으므로, exit 좌표를 (8-1)/(6-1) = 1.4배로 확장
...
4. **bonus row 추정 트레이드오프 (Codex §2-1 보강)**:
   - user A가 turn 3에 +3 받음 → 다른 user가 turn 3에 user A의 choice(좌/우)를 알면 그 turn의 보너스 row 추정 가능
...
- 상수 신설: `BRIDGE_COLUMNS=8`, `BRIDGE_MAX_WAVES=12`, `BRIDGE_MAX_SUDDEN_DEATH=6`, `BRIDGE_BONUS_AMOUNTS=[2,3]`, `BRIDGE_INTER_TURN_MS=1800`
...
### Stage 3 — 클라 world 확장 + layout (8 col)

- `Bridge` 생성자 `entrance/exit` 좌표를 (8-1)/(6-1)=1.4배 확장 (`js/bridge-cross.js:1407-1413`)
- `Bridge` `columnCount: 6 → 8`
- `tileSize` 그대로 유지 (450×214) — 시각 충돌 회피
- `Camera` worldW/worldH 자동 follow 검증 (zoom-out 동작 확인)
- 다리 col 8개 그리기 (`drawTile` 루프 자동 — `layout.columnCount` 사용 검증)
- `state.revealed` 길이 8로 변경 (`js/bridge-cross.js:2037, 3528`)
...
- turn 패널 라벨 한국어화 (`"턴 K/12"`, sudden death `"🔥 SUDDEN DEATH"`)
...
- 모두 한 번에 도달 (turn 1에 +3 받은 N명, 3 turn 안에 모두 progress=8 가능)
- max turn 12 도달 + 도달 못한 user 1명: 그가 꼴등 (sudden death 발동 X)
- max turn 12 도달 + 도달 못한 user 2명+: sudden death loop → 1명 남을 때까지 추가 turn
