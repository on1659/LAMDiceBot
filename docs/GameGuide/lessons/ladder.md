...
## 2026-09-05 (오후) — ⚠️ **5레인 경마화로 재교체** — v2 lesson 대부분 다시 도달 불가

같은 날 오전에 복원했던 v2(네이버식 라벨 매핑)를 걷어내고, **`9f82a1c`(2026-06-17)의 경마화 사다리**로
되돌린 뒤 4가지를 얹었다: 레인 5개 고정(번호 1~5) · 번호 **중복 선택 허용** · 인원 상한 없음 ·
바닥 표식은 「당첨」. 중복 허용 때문에 **동시 당첨 → 그 사람들끼리 30초 자동 재경기**가 새로 생겼다.
명세: `docs/goal/ladder-horse-style-5lane.md`

- 아래 v2 lesson들(perm 셔플 권위, 라벨 편집, 동시 하강 기본, living-rungs 변형)은 **현재 코드에서 도달 불가**다.
  `perm`·`topLabels`·`bottomLabels`·`labelLocks`·`mutationScript` 모두 제거됐다.
- **공정성 권위는 이제 `doReveal`의 "점유 레인 균등 추첨" 한 곳이다.** 최종 보드로 `laneToBottom`을 먼저
  구한 뒤, **사람이 고른 레인 중에서** 균등 랜덤으로 `winLane`을 뽑고 `winBottom = laneToBottom[winLane]`에
  당첨을 놓는다. 막대기는 결과에 개입하지 않는다(연출·긴장 담당).

### L-1. 당첨 후보는 **유니크 레인 집합**이어야 한다 — `Object.values`를 그대로 쓰면 몰린 번호가 유리해진다

- 중복 선택을 허용하면 `Object.values(ld.userLanes)`에는 같은 레인이 인원수만큼 들어간다.
  그 배열에서 균등 추첨하면 **3명이 몰린 번호의 당첨 확률이 3배**가 된다 — "많이 몰릴수록 잘 걸린다"는
  아무도 의도하지 않은 룰이 조용히 생긴다.
- **해결:** `[...new Set(...)]`으로 유니크화한 뒤 추첨한다. 그래야 레인 단위 균등(1/k)이 되고,
  "같은 번호를 고른 사람들은 함께 당첨"이라는 경마식 의미가 유지된다.
- **검증:** `AutoTest/qa-ladder-5lane-fairness-test.js` — 4명 중 3명이 한 번호에 몰린 상황에서
  그 번호의 당첨률이 50%(1/2)에 수렴하는지 확인한다(75%가 나오면 유니크화가 빠진 것).

### L-2. `pickLane`은 **토글**이다 — 자동 배정값과 겹치는 순간 반대로 동작한다

- 입장 시 서버가 빈 번호 하나를 자동 점유해준다(`claimLadderFreeLane`). 그 상태에서 같은 번호를 누르면
  선택이 아니라 **취소**다(설계된 동작).
- 그래서 테스트가 번호를 하드코딩해 `emit`하면, 자동 배정값과 우연히 겹치는 판에서만 취소로 동작해
  **간헐적으로만 깨지는 플레이키**가 된다. 스냅샷 폴링까지 겹치면 직전 상태로 즉시 통과해버려
  다음 단계가 어긋난다(원인 추적이 특히 어렵다).
- **해결:** 테스트는 "이미 그 번호면 누르지 않는" 멱등 setter(`setLane`)를 쓰고, 블라인드 `emit` 뒤에
  스냅샷을 폴링하지 않는다. 제품 UI는 토글이 맞으므로 서버는 그대로 둔다.

### L-3. 빈 레인 정리를 3경로 모두에서 하지 않으면 "주인 없는 번호"가 당첨된다

- 나간 사람의 `userLanes` 항목이 남아 있으면 그 레인이 계속 "사람 있는 레인"으로 잡혀 당첨 후보에 들어간다.
  결과적으로 **아무도 없는 번호가 당첨**되어 당첨자 0명이 된다.
- 정리 경로는 **3곳**이다(C-19와 같은 짝): `leaveRoom`(`socket/rooms.js`) · 진짜 disconnect(`socket/chat.js`) ·
  준비 취소(`socket/shared.js`의 `syncLadderBuildOnReadyChange`). 하나만 빠져도 특정 이탈 방식에서만 재현된다.
- **검증:** `tests/test-ladder.js`의 "빈 레인 당첨 금지 — 3판 반복"과 공정성 테스트의 200회 전수 확인.

### L-4. 옛 버전을 복원하면 **그 시점 이후에 생긴 셸 기능도 같이 사라진다**

- `9f82a1c`는 스티키 광고(2026-08-18 도입)보다 이전이라 `document.body.classList.add('race-running')`
  토글이 아예 없었다. 게임 로직만 보고 복원하면 **연출 중 광고가 캔버스를 덮는 회귀**가 조용히 따라온다.
- 같은 이유로 클라 셸(진입 IIFE·비밀번호 재입장·`ladderEnterRoom`)은 **현재 것을 유지**하고 게임 로직만
  이식해야 한다. 옛 파일을 통째로 덮으면 dice-lobby 진입 배선(현재 14곳)이 통째로 날아간다.
- **해결:** 복원 전에 `git log --oneline <복원기준>..HEAD -- <파일>`로 그 사이에 들어온 기능을 목록화하고,
  각각 "룰인가 셸인가"를 판정해 셸은 살린다.
- **검증:** 공유 e2e(`qa-sticky-ad-race-toggle-test.js`)를 반드시 돌린다 — 이 회귀는 게임 화면만 봐서는 안 보인다.

### L-5. 재경기는 예약 발화를 쓴다 — `SUPPORTED_GAME_TYPES` 등록과 `canStart`/`start` 계약이 필요하다

- 동시 당첨 재경기는 경마와 같이 `gameState.scheduledStartAt` + `scheduled-start.js` 스위퍼로 시작한다.
  그런데 스위퍼는 `SUPPORTED_GAME_TYPES[gameType]`으로 모듈을 찾으므로, **등록하지 않으면 발화 순간
  `require(undefined)`로 조용히 죽고 예약만 사라진다**(방장이 안 누르면 영영 시작 안 됨).
- 또 스위퍼가 넘기는 ctx는 `{ rooms, updateRoomsList }`뿐이다(`socket/index.js`). 소켓 클로저에 담아둔
  헬퍼(`ctx.emitLadderRungsUpdated` 같은)에 기대면 안 되고, **모듈 레벨 `canStart`/`start`**를 노출해야 한다.
- **검증:** 재경기 판이 방장 조작 없이 실제로 시작되는지 e2e로 확인(`tests/test-ladder.js` 마지막 케이스).
