...
## C-45. "고쳤는데 그대로예요"는 코드보다 **어느 서버 프로세스를 보고 있는지**부터 확인하라

- `node server.js`는 자동 리로드가 없다. `socket/*`·`utils/*`·`db/*` 같은 서버 파일을 고치면 **프로세스를 다시 띄워야** 반영된다. 정적 파일(`*.html`, `js/`, `css/`)만 바뀐 경우는 서버가 디스크에서 새로 읽으므로 새로고침이면 된다(단, `?v=` 캐시 버전은 올려야 한다).
- 한 폴더에서 세션마다 서버를 따로 띄우는 환경(5173/5174/5175…)에선 **다른 포트에 옛 프로세스가 남아 있는 것**이 "재시작해도 안 고쳐진다"의 1순위 원인이다. 사용자가 5173을 껐다 켜도 브라우저가 5174를 보고 있으면 아무 소용이 없다. 증상은 "새 이벤트를 보내도 아무 반응이 없다"(옛 서버에 핸들러가 없어 조용히 무시) + "새 단계 없이 예전 흐름으로 바로 넘어간다".
- **진단 순서(코드 뒤지기 전에):**
  ```bash
...
- (출처: 2026-09-21 데구리 당첨 순위 투표 — 5173은 두 번 재시작됐지만 5174는 18:54 프로세스가 4시간 남아 "투표가 안 그려지고 바로 3,2,1" 제보. 메모리 `project_dev_server_restart_for_socket` 의 실전 사례)
