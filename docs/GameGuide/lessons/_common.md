...
### 확장 ① — 키뿐 아니라 **값**도 맞아야 억제된다 (`'true'`는 무시된다)

- 억제 판정은 `localStorage.getItem('tutorialSeen_' + gameType) === VERSION`이다(`tutorial-shared.js`).
  즉 값이 현재 `VERSION` 문자열(2026-09-05 기준 `'v1'`)과 **정확히 같아야** 한다.
- 그래서 테스트에서 흔히 쓰는 `setItem(key, 'true')`는 **아무 효과가 없다.** 키 이름이 맞아도
  튜토리얼이 그대로 떠서 click-blocker에 클릭이 막힌다 — C-28 본문과 똑같은 증상이 값 때문에 재현된다.
- **해결:** `localStorage.setItem('tutorialSeen_' + game, 'v1')`처럼 VERSION 값을 심는다.
  하드코딩이 싫으면 `tutorial-shared.js`의 VERSION을 읽어 파생시켜라.

### 확장 ② — `VERSION`은 **전 게임 공용 상수**다. 한 게임 때문에 올리지 마라

- 룰을 갈아엎은 게임의 튜토리얼을 다시 보여주려고 `VERSION`을 올리고 싶어지지만, 이 상수는
  게임별이 아니라 **하나뿐**이다. 올리는 순간 주사위·룰렛·경마까지 **모든 사용자에게** 튜토리얼이 다시 뜬다.
- **해결:** 한 게임의 튜토리얼만 재노출해야 하면 게임별 버전 분리를 먼저 설계한다. 그 값어치가 없으면
  **문구만 교체하고 버전은 그대로 둔다**(기존 사용자는 도움말 버튼으로 새 문구를 볼 수 있다).
- (출처: 2026-09-05 사다리 5레인 경마화 — 룰 전면 교체 후 버전 상향을 검토하다 공용 상수임을 확인하고 철회)
...
## C-45. 공유 e2e 테스트의 FAIL은 **origin/main 실측**으로 회귀/기존을 갈라라

- 여러 게임을 한 파일에서 도는 공유 테스트(`AutoTest/qa-sticky-ad-race-toggle-test.js` 같은)는
  내 작업과 무관한 항목도 함께 실패한다. 그 FAIL을 내 변경 탓으로 오해하면 멀쩡한 코드를 뜯어고치게 되고,
  반대로 "원래 그랬겠지"로 넘기면 진짜 회귀를 놓친다. **둘 다 실제 사고 사례다.**
- **해결(순서):**
  1. `git status`로 그 실패 항목이 건드리는 파일을 내가 수정했는지 먼저 본다. 안 건드렸으면 강한 pre-existing 신호다.
  2. 그래도 확실치 않으면 `git worktree add <임시경로> origin/main`으로 깨끗한 트리를 만들고
     거기서 같은 테스트를 돌려 **같은 항목이 같은 이유로 실패하는지** 실측한다.
  3. pre-existing이면 보고에 "기존 이슈"로 명시하고 범위 밖으로 둔다. 조용히 넘기지 않는다.
- 실제 사례: 해적 `P2(G) 재입장 후 숨김 미복원` + 해적 페이지 404 콘솔 에러 5건은 사다리 작업과 무관한
  기존 실패였다(해적 파일 무변경으로 확인). 사다리 `L1`은 진짜 회귀였다(복원한 옛 클라에 `race-running`
  토글이 없었음) — 같은 실행 안에서 둘이 섞여 있었다.
- **검증:** 보고에 "FAIL n건 중 회귀 x건 / 기존 y건"과 그 판정 근거를 함께 적는다.
- (출처: 2026-09-05 사다리 5레인 경마화)

---

## C-46. 승패가 없는 게임은 참가자 `is_winner` 행을 만들지 마라 — 크로스게임 승률의 분모가 오염된다

- `db/ranking.js`의 크로스게임 승률은 `COUNT(*) FILTER (WHERE is_winner) / COUNT(*)`를 **game_type 필터 없이**
  계산한다. 그래서 승패 개념이 없는 게임이 "전원 `is_winner=false`" 행을 넣으면 분자는 그대로인데
  분모만 불어난다 → **그 게임을 할수록 모든 유저의 종합 승률이 내려간다.** 게임 자체엔 아무 증상이 없어
  랭킹을 들여다보기 전엔 안 잡힌다.
- **해결:** 승자가 정의되지 않는 게임은 `recordGamePlay` + `recordGameSession`(`winnerName: null`)만 남기고
  개인별 `recordServerGame` 행은 넣지 않는다.
- **뒤집히는 조건:** 나중에 그 게임에 승자 개념이 생기면 이 판단도 같이 뒤집어야 한다. 실제로 사다리는
  2026-09-05 이후 「당첨자」가 생겨 **개인 행을 기록하는 쪽으로 바뀌었다**(그래야 사다리 랭킹 탭이 채워진다).
  즉 이 lesson은 "승패 없는 게임"에만 적용된다 — 게임 룰이 바뀌면 기록 정책도 함께 재검토하라.
- **검증:** 새 게임의 DB 기록을 넣기 전에 "이 게임에 승자가 정의되는가?"를 먼저 답하고, 아니면 세션만 남긴다.
- (출처: 2026-09-05 사다리 — 적대 리뷰 F1에서 발견, 같은 날 룰 변경으로 정책이 뒤집힘)

---

## C-47. 스티키 광고는 짧은 화면에서 캔버스 하단을 덮는다 — AutoTest 좌표 조작은 스크롤 보정을 넣어라

- 하단 고정 광고(`.ad-container.ad-sticky`, 높이 ~96px)는 문서 흐름 밖에 떠 있다. 뷰포트가 짧으면
  캔버스 아래쪽이 그 밑으로 들어가고, Playwright의 좌표 기반 조작(`mouse.move/down/up`으로 막대기 드래그 등)이
  **광고를 눌러버려** 아무 일도 안 일어난다. 요소는 존재하고 `boundingBox()`도 정상이라 원인을 찾기 어렵다.
- **해결:** 좌표 드래그 전에 대상 캔버스를 `scrollIntoView({ block: 'center' })`로 화면 중앙에 올리고,
  `boundingBox()`를 **스크롤 이후에 다시 읽는다**(스크롤 전 좌표는 이미 낡았다).
  뷰포트를 충분히 크게 잡는 것(예: 1280×900)도 함께 쓴다.
- 실사용 UI에도 같은 문제가 있으니, 하단에 조작 대상이 오는 화면은 `body`에 스티키 높이만큼
  `padding-bottom`이 실제로 걸려 있는지 확인한다(연출 중 `body.race-running`으로 숨길 때는 0px).
- **검증:** 드래그 직후 상태 변화(막대기 수 증가 등)를 단언한다. "클릭했다"가 아니라 "바뀌었다"를 본다.
- (출처: 2026-09-05 사다리 QA — 캔버스 드래그가 조용히 실패)
