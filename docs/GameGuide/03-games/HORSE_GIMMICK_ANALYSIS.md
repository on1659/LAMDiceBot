...
| **ITEM_ROCKET (로켓)** | 6% | 1.9-2.3x | 400-700ms | 🚀✨ + 스피드라인 |
| **ITEM_ICE (얼음)** | 6% | 0.15x | 800-1400ms | ❄️ + 빙결 슬라이드 |
...
### ITEM_ROCKET (로켓)
- 로켓을 획득하여 폭발적인 초단기 부스트 (비진화 부스트 중 최강 배율)
- 🚀✨ 이모지 + 반짝임 애니메이션 + 스피드 라인
- CSS 필터: `brightness(1.6) saturate(1.8)` 적용
- 가장 짧은 지속시간(400-700ms)으로 밸런스 조정됨

### ITEM_ICE (얼음)
- 얼어붙어 쉬는 자세로 아주 천천히 미끄러짐 (0.15x 빙결 슬라이드)
- ❄️ 이모지 + 떨림(iceShiver) 애니메이션
- CSS 필터: `saturate(0.2) brightness(1.3)` 적용
- 완전정지(STOP/OBSTACLE)와 달리 미세하게 전진
