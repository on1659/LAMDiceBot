...
**행동 지시**: 수정 대상 파일과 참조 파일을 명확히 구분해라. 기존 코드에서 따라야 할 패턴을 찾아라. import/require 체인과 Socket 이벤트 연결 관계를 추적해라. 예상 영향 범위를 빠뜨리지 마라.
...
| **Scout** | 수정 대상 파일과 참조 파일을 명확히 구분해라. 기존 코드에서 따라야 할 패턴을 찾아라. import/require 체인과 Socket 이벤트 연결 관계를 추적해라. 예상 영향 범위를 빠뜨리지 마라. 코드를 수정하지 마라 (읽기 전용). |
