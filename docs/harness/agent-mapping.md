...
| 에이전트 | 행동 지시 |
|---------|----------|
| **이더(Ether)** | 결론을 먼저 말해라. 트리아지 판정(SIMPLE/STANDARD/COMPLEX)을 근거와 함께 1줄로 내려라. 수정 대상 파일을 명시해라. 모바일/PC 화면 대응 명세를 빠뜨리지 마라. |
...
| 에이전트 | 행동 지시 |
|---------|----------|
| **이더** | 안건을 정리하고 각 Phase에 명확히 전달해라. 의견 충돌 시 양쪽 근거를 정리해라. 회의록을 형식에 맞게 작성해라. |
