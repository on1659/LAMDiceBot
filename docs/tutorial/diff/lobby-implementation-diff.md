...
| | 설계 | 실제 |
|---|------|------|
| 방향 | 고정 (`left: 50%`), 상하좌우 4방향 | **동적 계산** (`--ax` CSS 변수), **상하 2방향만** |
| 네이밍 | `arrow-bottom` (화살표가 아래 방향) | `arr-bottom` (툴팁이 아래에 위치) |
| 좌우 | `arrow-left`, `arrow-right` 지원 | **삭제** — 상하 전용 |

**교훈**: 좌우 배치는 모바일에서 거의 항상 flip되거나 짤려서 제거. 상하만으로 충분. 동적 `--ax` 계산으로 화살표가 대상 중심을 정확히 가리킴.
...
| | 설계 | 실제 |
|---|------|------|
| 방향 | 상하좌우 4방향, clamp만 | **상하 전용**, flip + clamp |
| 좌우 배치 | `position: 'left'/'right'` 지원 | **제거** — 좌우 입력해도 상하로 배치 |
| PC/모바일 분기 | 없음 | **없음** (통합 로직, 모바일은 CSS만 별도) |

**교훈**: 좌우 flip 분기가 있으면 코드 복잡도가 2배. 상하만으로 충분하고 모바일에서도 안정적. PC/모바일 같은 JS 로직에 모바일 CSS(`@media 480px`)만 별도 적용.

### 1-8. 모바일 대응

| | 설계 | 실제 |
|---|------|------|
| 언급 | "모바일 320px — clamp" 한 줄 | `@media (max-width: 480px)` CSS만 (JS 분기 없음) |
| CSS | 없음 | 폰트/패딩 축소, 오버레이 투명도 감소, width auto |
| 배치 | 동일 로직 | **PC/모바일 통합** — 상하 전용 로직이 양쪽 모두 대응 |

**교훈**: 상하 전용 배치 + flip/clamp 로직으로 PC/모바일 JS 분기가 불필요해짐. 모바일은 CSS(`@media 480px`)로 폰트/패딩만 축소하면 충분.
