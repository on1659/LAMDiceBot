// 사다리타기(ladder) 게임 소켓 핸들러 — vibe(D:\Work\vibe\ladder) 메커니즘 (v2 복원, 2026-09-05).
// 이력: v2(53656e1) → pick-elimination(f7cafa1)으로 교체됐다가 실플레이 판정("원래 v2가 잘 되던 것")으로
//   v2를 복원. 명세: docs/goal/ladder-v2-restore.md. 복원 시 반영: points 깊은 복사(양 사이트),
//   그리기 예산 제거, 바닥 라벨 셔플 연출 단계(vibe 현재 기준), 동시 하강 기본값, 재진입 정밀 복구.
...
const { recordGameSession, generateSessionId } = require('../db/servers');
...
const LADDER_SHUFFLE_MS = 3200;       // 바닥 라벨 셔플 연출(클라 ladderRunShuffle) — 스크램블 뒤·카운트다운 앞 직렬, ladderRevealDelay 합산. js/ladder.js와 byte-identical.
...
//   스크램블 = ERASE + DRAW(시각, 매핑 무관). 셔플 = SHUFFLE_MS(바닥 라벨 위치 섞기 연출 — 스크램블 뒤·카운트다운 앞).
//   하강 = descentSlots × TOKEN_SLOT. 변형 = mutations × MUTATION_MS.
...
// (공유 그리기 예산 제거 — 2026-09-05 명세 결정: 인당 cap 3(FIFO)만 유지.
//  예산이 인원수와 무관해 선착이 소진하면 후입장자가 못 그리는 소외가 생겼고, vibe 현재 버전도 예산을 제거함.)
...
function buildLadder(N, baseRungs, userRungsMap, nextId) {
    ...
    //    ⚠ points는 양 사이트(user/base) 모두 깊은 복사 — resolveContacts가 union 항목의 points[0]/[last]를
    //      in-place 재배정하므로, 참조 공유 시 원본 ld.userRungs/ld.baseRungs의 그려진 선이 실제로 변조된다
    //      ("내가 그린 선이 달라졌다" — fef0280 계열, 복원 시 양 사이트로 확장). 단 union 내부의
    //      added↔rungs 참조 공유(4-b)는 유지한다(스크램블 그리기 연출 동기의 전제 — 새 객체로 교체 금지).
    const union = [];
...
module.exports = (socket, io, ctx) => {
    ...
    function emitRungsUpdated(room, gameState) {
        ...
        io.to(room.roomId).emit('ladder:rungsUpdated', {
            ...
            descentMode: ld.descentMode || 'simultaneous'
        });
    }
    ...
    // ① 모든 phase: ladder:stateSync { labelEditMode, history } — 편집 권한 모드(비-idle 재진입 시 rungsUpdated가
    //    안 와 'all'로 표류하던 결함, 적대 리뷰 F10)와 게임 기록(마스킹 후 재전달 경로가 없어 새로고침 시
    //    유실되던 결함, F8)을 복원한다. history는 이미 방 전체 공개 정보(gameEnd 브로드캐스트 누적)라 누출 아님.
    // ② revealing/finished: ladder:reveal + elapsedMs 재전송(진행 중 연출을 그 지점부터 seek).
    //    공정성: reveal payload는 브로드캐스트 순간부터 방 전체 공개 — 재전송은 C-20의 의도된 예외
    //    (docs/goal/ladder-v2-restore.md). finished면 gameEnd 스냅샷도 재전달.
    function emitLadderStateSync(room, gameState, socketId) {
        ...
        if (ld.phase === 'finished') {
            // 결과 화면 복원 — gameEnd 스냅샷(이미 방 전체에 공개된 정보의 재전달).
            io.to(socketId).emit('ladder:gameEnd', {
                round: ld.round,
                numColumns: (ld.topLabels || []).length,
                topLabels: (ld.topLabels || []).slice(),
                bottomLabels: (ld.bottomLabels || []).slice(),
    ...
    function runLadder(room, gameState) {
        ...
        // 참가자 스냅샷 — DB 기록(endGame)은 이 시작 시점 명단을 쓴다(연출 중 입장/이탈 오염 방지).
        ld.participantsAtStart = (gameState.users || []).map(u => u.name);
        ...
        if ((ld.descentMode || 'simultaneous') === 'simultaneous') {
        ...
        // ld.revealPayload/revealStartAt은 server-only지만 공정성 비밀이 아니다(브로드캐스트 순간부터 방 전체 공개
        // — C-20의 의도된 예외). getCurrentRoom은 ladder를 통째 마스킹하므로 어차피 새지 않는다.
        const revealPayload = {
            numColumns: N,
            ...
            perm: perm.slice(),                                // 셔플 순열(슬롯 j ← 원본 라벨 인덱스 perm[j]) — 표시 전용(클라 라벨 셔플 연출). ld에 저장 금지(idle 재동기 누설 차단 — revealPayload에만 존재)
            descentMode: ld.descentMode || 'simultaneous'      // 내려가기 방식(클라 ladderRun.descentMode로 저장 — 연출 분기)
        };
        ...
        }, ladderRevealDelay(N, ld.descentMode || 'simultaneous'));
    ...
    function endGame(room, gameState) {
        ...
        // ── DB 기록 (중립 매핑 — 단일 패자 없음) ──
        // 참가자 = "시작 시점" 스냅샷(ld.participantsAtStart, runLadder가 저장). 연출 ~수십 초 동안의
        // 입장/이탈이 기록을 오염하지 않게 한다(적대 리뷰 F2 — 관전 입장자가 기록되고 실참가 이탈자가 빠지던 결함).
        const participants = (ld.participantsAtStart && ld.participantsAtStart.length)
            ? ld.participantsAtStart.slice()
        ...
        if (room.serverId) {
            ...
            // 개인별 server_game_records는 기록하지 않는다(적대 리뷰 F1). 매핑 룰엔 승자/패자가 없는데
            // 전원 is_winner=false 행을 넣으면 크로스게임 승률(win/COUNT(*), game_type 필터 없음)의
            // 분모만 불려 사다리를 할수록 승률이 내려간다. 세션 1행(winner_name null)만 남긴다.
            recordGameSession({
    ...
    function resetLadder(ld) {
        ...
        ld.participantsAtStart = [];  // server-only: DB 기록용 시작 시점 참가자 스냅샷 폐기
        ld.isLadderActive = false;
    ...
    socket.on('ladder:addRung', (data) => {
        ...
        // (공유 그리기 예산은 제거 — 인당 cap만 남김. docs/goal/ladder-v2-restore.md)
    ...
    socket.on('ladder:setColumns', (data) => {
        ...
        // 방장만 편집 모드에선 칸 수 변경도 방장 전용 — 칸 축소가 라벨 절단·타인 막대기 트림을 유발하므로
        // setLabel의 host-only 보호를 setColumns로 우회할 수 있었다(적대 리뷰 F4).
        if (ld.labelEditMode === 'host') {
            const me = gameState.users.find(u => u.id === socket.id);
            if (!me || me.isHost !== true) {
                socket.emit('ladder:error', '방장만 칸 수를 바꿀 수 있어요. (방장만 편집 모드)');
                return;
            }
        }
    ...
    socket.on('ladder:setLabel', (data) => {
        ...
        // 소프트락 존중 — 남이 락을 쥔 칸은 setLabel도 거부(적대 리뷰 F7: 락이 labelTyping에만 걸려 있어
        // 직접 emit으로 편집 중인 칸을 덮어쓸 수 있었다). 락 보유자 본인/락 없음은 통과.
        {
            const lockKey = data.side + ':' + idx;
            const lock = ld.labelLocks && ld.labelLocks[lockKey];
            if (lock && (!me || lock.name !== me.name)) {
                socket.emit('ladder:error', lock.name + ' 님이 입력 중인 칸이에요.');
                return;
            }
        }
    ...
    socket.on('ladder:labelFocus', (data) => {
        ...
        // hostOnly 플래그: 실제 락이 아니라 권한 거부다 — 클라가 이 deny를 락으로 저장하면
        // 모드를 all로 되돌려도 unlock이 영영 안 와 그 칸이 고착된다(적대 리뷰 F9).
        if (ld.labelEditMode === 'host' && me.isHost !== true) {
            socket.emit('ladder:labelLockDenied', { side: data.side, index: idx, name: (gameState.users.find(u => u.isHost) || {}).name || '', hostOnly: true });
            return;
        }
...
// 타이밍 export — 테스트 워치독을 서버 상수에서 파생(하드코딩 금지, lesson 2026-08-31)
module.exports.ladderRevealDelay = ladderRevealDelay;
module.exports.LADDER_SHUFFLE_MS = LADDER_SHUFFLE_MS;
module.exports.LADDER_COUNTDOWN_MS = LADDER_COUNTDOWN_MS;
module.exports.LADDER_ERASE_MS = LADDER_ERASE_MS;
module.exports.LADDER_DRAW_MS = LADDER_DRAW_MS;
module.exports.LADDER_TOKEN_SLOT_MS = LADDER_TOKEN_SLOT_MS;
module.exports.LADDER_MUTATION_MS = LADDER_MUTATION_MS;
module.exports.LADDER_FINAL_HOLD = LADDER_FINAL_HOLD;
