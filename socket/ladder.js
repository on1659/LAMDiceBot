// 사다리타기(ladder) 게임 소켓 핸들러 — "경마인데 과정이 사다리" 메커니즘.
// 기준점: 9f82a1c(2026-06-17)의 경마화 사다리를 복원한 것. 53656e1(v2 네이버식)과 f7cafa1(pick-elimination)은 폐기.
// 명세: docs/goal/ladder-horse-style-5lane.md
// 9f82a1c 대비 바뀐 것: 레인 6→5, 번호 중복 선택 허용, 인원 상한 없음, 「꽝」→「당첨」,
//   중복 당첨 시 재경기(경마 패턴), 마지막 두 토큰 동시 하강, 재진입 복구(v2에서 이식).
// 결과(winLane·winBottom·laneToBottom·revealOrder)는 전적으로 서버가 결정 — 클라는 받은 값으로 연출만(공정성).
const { DISCONNECT_WAIT_REDIRECT, DISCONNECT_WAIT_DEFAULT, LADDER_REMATCH_AUTO_START_MS } = require('../config');
...
const LADDER_LANES = 5;             // 레인 항상 5개 고정(번호 1~5). 아무도 안 고른 번호는 주인 없는 빈 레인.
// 인원 상한 없음 — 번호 중복 선택을 허용하므로 5명을 넘어도 된다(경마와 동일). 한 레인에 여러 명이 걸리면
// 그 사람들은 같은 토큰을 공유하고, 그 레인이 당첨되면 다 함께 당첨 → 재경기로 한 명을 가린다.
...
// 같은 시간(SLOT_MS)에 주파한다. 단, **마지막 두 토큰은 한 슬롯을 공유하며 같이** 내려간다
// → 총 하강 시간 = ladderDescentSlots(N) × SLOT_MS.
...
const LADDER_BOMB_POINTER_MS = 5200;  // 폭탄 룰렛 포인터가 바닥칸을 가속→감속하며 훑다 winBottom(당첨 칸)에 정지하는 시간(하강 전) (2600×2) — js/ladder.js와 동일 유지
const LADDER_FINAL_HOLD = 1800;       // 결과 캡션 유지(ms) — 모션 아님, 유지 — js/ladder.js와 동일 유지

// reveal 시작부터 자동 종료(결과 오버레이)까지 — 순차 하강이라 토큰 수(N)에 비례.
// 시퀀스(당첨 선결정): 카운트다운 → 지우기 → 그리기 → 바닥멈춤 → 폭탄 포인터(당첨 칸 공개) → 순차 하강 → 결과 캡션 유지.
// 모든 단계 길이를 합산해야 서버 endGame이 클라 연출 도중에 끼어들지 않는다(서버↔클라 타이밍 동기 불변).
// N = 점유 레인(하강 토큰) 수.
//
// 하강 슬롯 = (N <= 1 ? N : N - 1) — 마지막 두 토큰은 한 슬롯을 공유하며 **같이** 내려간다.
// 9f82a1c는 마지막 토큰도 혼자 내려가서(N개 슬롯) 마지막 한 명의 결과가 그 전에 이미 확정돼 김이 샜다.
// 소유자 요청으로 v2의 "마지막 쌍 동시 하강"만 가져왔다. js/ladder.js의 ladderDescentSlots와 반드시 동일 유지.
function ladderDescentSlots(N) {
    const n = Math.max(0, N | 0);
    return n <= 1 ? n : n - 1;
}
function ladderRevealDelay(N) {
    const descent = ladderDescentSlots(Math.max(1, N | 0)) * LADDER_TOKEN_SLOT_MS;
 ...
 * winBottom/winLane/winners는 여기서 정하지 않는다(doReveal의 점유 레인 균등 랜덤이 권위).
 * @param {number} N - 레인 수(고정 6)
...
function buildLadder(N, baseRungs, userRungsMap, nextId) {
    ...
    // winBottom/winLane/winners는 doReveal에서 점유 레인 균등 랜덤으로 확정(공정성 불변).
 ...
 */
// ─── 게임 엔진 ───
// socket과 무관한 부분(io/ctx만 의존)을 한데 모은다. 소켓 등록부와 예약 발화(module.exports.start)가
// 같은 엔진을 쓴다 — 예약 스위퍼의 ctx는 { rooms, updateRoomsList }뿐이라(socket/index.js) 소켓 클로저에
// 기댈 수 없기 때문이다. 상태는 전부 gameState에 있으므로 매 호출마다 새로 만들어도 안전하다.
function createLadderEngine(io, ctx) {
    const updateRoomsList = ctx.updateRoomsList || (() => {});
    ...
    function claimFreeLane(ld, N, name) {
        ...
        if (!freeLanes.length) return false;   // 5개 다 참 — 미배정으로 두고, 직접 고르거나 시작 시 자동 배정된다
    ...
    // server-only 정보(final rungs/laneToBottom/winLane/winBottom/winners/erased/added)는 미포함.
    ...
    // 재진입/재연결 동기화 — 방에 (재)입장한 소켓에게 개인 emit. rooms.js(joinRoom 신규/슬롯 인계)가 호출.
    // ① 모든 phase: ladder:stateSync { history } — 게임 기록은 마스킹 후 재전달 경로가 없어 새로고침 시 유실된다.
    //    history는 이미 방 전체 공개 정보(gameEnd 브로드캐스트 누적)라 누출이 아니다.
    // ② revealing/finished: ladder:reveal + elapsedMs 재전송(진행 중 연출을 그 지점부터 seek).
    //    reveal payload는 브로드캐스트 순간부터 방 전체 공개 — 재전송은 ladder 통째 마스킹의 의도된 예외.
    //    finished면 gameEnd 스냅샷도 재전달(결과 화면 복원).
    function emitLadderStateSync(room, gameState, socketId) {
        const ld = gameState && gameState.ladder;
        if (!ld || !socketId) return;
        io.to(socketId).emit('ladder:stateSync', {
            // 서버는 오래된 순(push) — 클라 표시 계약은 최신 우선이라 여기서 뒤집어 보낸다.
            history: (ld.ladderHistory || []).slice().reverse()
        });
        if (ld.phase !== 'revealing' && ld.phase !== 'finished') return;
        if (!ld.revealPayload) return;   // 진행 중 payload 없음(서버 재시작 등) — 동기화 불가, 다음 라운드부터 정상
        const elapsedMs = ld.revealStartAt ? Math.max(0, Date.now() - ld.revealStartAt) : Number.MAX_SAFE_INTEGER;
        io.to(socketId).emit('ladder:reveal', { ...ld.revealPayload, elapsedMs });
        if (ld.phase === 'finished') {
            io.to(socketId).emit('ladder:gameEnd', {
                winners: (ld.winners || []).slice(),
                rankings: Object.entries(ld.userLanes || {}).map(([name, lane]) => ({
                    name, lane, bottom: (ld.laneToBottom || [])[lane], isWinner: (ld.winners || []).includes(name)
                })),
                winLane: ld.winLane,
                winBottom: ld.winBottom,
                round: ld.round
            });
        }
    }
    ctx.emitLadderStateSync = emitLadderStateSync;
    ...
    function doReveal(room, gameState) {
        ...
        // 미선택 참가자 자동 배정 (경마 패턴) — 빈 레인을 서버 RNG로 섞어 먼저 나눠주고,
        // 빈 레인이 동나면(인원 > 5) 아무 레인이나 균등 배정한다. 중복 허용이라 만석이어도 배정이 실패하지 않는다.
        ...
        ld.participants.forEach(name => {
            if (ld.userLanes[name] === undefined && gameState.users.some(u => u.name === name)) {
                ld.userLanes[name] = (fi < freeLanes.length)
                    ? freeLanes[fi++]
                    : Math.floor(Math.random() * ld.numLanes);
                assignColorIndex(ld, name);
                autoAssigned.push(`${name}님(${ld.userLanes[name] + 1}번)`);
            }
        });
        if (autoAssigned.length) {
            require('./scheduled-start').roomNotice(io, room, gameState,
                `${autoAssigned.join(', ')}이 번호를 고르지 않아 자동으로 배정했어요.`);
        }

        // 당첨(winLane)은 반드시 **사람이 고른 레인 중에서만** 뽑는다 — 5레인 고정이라 빈 레인이 있어도
        // "당첨자 없는 판"이 생기지 않게 한다. 후보는 **유니크 레인 집합**이어야 한다:
        //   Object.values를 그대로 쓰면 3명이 몰린 레인이 후보에 3번 들어가 그 레인의 당첨 확률이 3배가 된다.
        //   레인 단위 균등(1/k)이어야 "같은 번호를 고른 사람들이 함께 당첨"이 경마의 같은 말 베팅과 같아진다.
        // winBottom은 그 레인의 도착 바닥칸(laneToBottom 매핑 그대로 — 시각/매핑 불변).
        // 막대기는 한 개도 조작하지 않는다: 최종 보드로 매핑을 먼저 구하고, 그 위에서 당첨 카드 위치만 정한다.
        const occupiedLanes = [...new Set(Object.values(ld.userLanes).filter(l => typeof l === 'number'))];
        if (occupiedLanes.length) {
            ld.winLane = occupiedLanes[Math.floor(Math.random() * occupiedLanes.length)];
            ld.winBottom = ld.laneToBottom[ld.winLane];
        } else {
            ld.winLane = -1;
            ld.winBottom = -1;
        }
        ...
        // 당첨자 = winLane을 고른 사람 전원(중복 허용이므로 여러 명일 수 있다). 2명 이상이면 endGame이 재경기를 건다.
        ld.winners = Object.keys(ld.userLanes).filter(name => ld.userLanes[name] === ld.winLane);

        // 하강 순서 = 점유된 레인을 서버 RNG로 셔플. 시각 효과일 뿐 결과와 무관.
        // 토큰은 레인당 하나다(같은 레인을 고른 사람들은 한 토큰을 공유) → 레인 기준으로 만든다.
        // 모든 탭이 동일 순서로 재생하도록 페이로드에 포함(클라 Math.random 미사용 → 공정성 유지).
        const revealOrder = occupiedLanes.slice();
        ...
        // reveal payload — 브로드캐스트와 재진입 개인 재전송(emitLadderStateSync)이 같은 객체를 쓴다.
        // 브로드캐스트 순간부터 방 전체 공개이므로 재전송은 누출이 아니다(getCurrentRoom은 ladder를 통째 마스킹).
        const revealPayload = {
            numLanes: ld.numLanes,
            rungs: ld.rungs,                    // final(스크램블 후, y정렬). 각 막대기에 id·user·owner 포함(클라 색·라벨용)
            winBottom: ld.winBottom,            // 당첨 바닥칸 — 폭탄 포인터가 여기에 정지한다
            winLane: ld.winLane,
            laneToBottom: ld.laneToBottom,
            userLanes: { ...ld.userLanes },
            revealOrder: revealOrder,           // 하강 순서(레인 인덱스 배열) — 마지막 두 개는 같이 내려간다
            winners: (ld.winners || []).slice(),
        ...
        ld.revealStartAt = Date.now();          // server-only — 재진입 seek의 벽시계 원점
        ld.revealPayload = revealPayload;       // server-only — 재진입 개인 재전송용(resetLadder가 폐기)
        io.to(room.roomId).emit('ladder:reveal', revealPayload);

        console.log(`[사다리타기] 방 ${room.roomName} 공개 - 레인=${ld.numLanes}, 당첨바닥=${ld.winBottom}, 당첨레인=${ld.winLane}, 당첨자=${(ld.winners || []).join('/') || '없음'}`);

        clearLadderTimers(ld);
        // 종료 타이머 = 클라 연출 합과 정확히 일치. N = 점유 레인 수(= 하강 토큰 수 = revealOrder 길이).
    ...
    function endGame(room, gameState) {
        ...
        // 당첨자 확정 — winLane을 고른 사람 중 아직 방에 있는 사람. 중복 선택을 허용하므로 여러 명일 수 있다.
        // reveal 시점 명단(ld.winners)을 쓰되, 연출 중 나간 사람은 뺀다(빠져서 0명이 되면 그대로 0명 — 판은 무효처럼 넘어간다).
        const winners = (ld.winners || []).filter(name => gameState.users.some(u => u.name === name));

        // 결과 표: 각자 고른 번호와 도착 칸, 당첨 여부
        ...
        if (room.serverId) {
            // 경마와 같은 시점·같은 규칙으로 기록한다(socket/horse.js). 재경기 라운드도 매번 남기고,
            // 당첨자가 여러 명인 판은 그 전원이 is_winner=true로 들어간 뒤 다음 판에서 최종 1명이 다시 들어간다.
            ...
            Promise.all(players.map(name => {
                ...
                return recordServerGame(room.serverId, name, `${ld.userLanes[name] + 1}번`, 'ladder', isWinner, sessionId, rank);
            })).then(() => recordGameSession({
                serverId: room.serverId,
                sessionId,
                gameType: 'ladder',
                gameRules: 'ladder-5lane',
                winnerName: winners.length === 1 ? winners[0] : null,   // 중복 당첨 판은 아직 최종 승자가 아니다
                participantCount: players.length
            })).catch(e => console.warn('[사다리타기] DB 기록 실패:', e.message));
        }

        console.log(`[사다리타기] 방 ${room.roomName} 종료 - 당첨=${winners.join('/') || '없음'} (${ld.winLane + 1}번)`);

        // 당첨자가 2명 이상이면 그 사람들끼리 한 판 더 — 경마 재경기(socket/horse.js)와 같은 구조.
        // 서버가 당첨자만 준비 상태로 만들고, 예약 발화(scheduled-start)가 30초 뒤 [시작]을 대신 누른다.
        // 나머지는 준비가 풀려 관전이 된다. 번호는 아래 자동 리셋에서 비워지므로 다시 고른다.
        const isRematch = winners.length >= 2;
        if (isRematch) {
            const scheduled = require('./scheduled-start');
            gameState.readyUsers = winners.slice();
            gameState.users.forEach(u => { u.isReady = winners.includes(u.name); });
            io.to(room.roomId).emit('readyUsersUpdated', gameState.readyUsers);
            winners.forEach(name => {
                const u = gameState.users.find(x => x.name === name);
                if (u) io.to(u.id).emit('readyStateChanged', { isReady: true });
            });
            ...
            scheduled.roomNotice(io, room, gameState,
                `${winners.join(', ')}님이 같은 번호로 동시 당첨! ${Math.round(LADDER_REMATCH_AUTO_START_MS / 1000)}초 뒤 두 분끼리 한 판 더 시작해요. 번호를 고르지 않으면 자동으로 배정돼요.`);
        }

        // 게임 종료 → 바로 주문받기 자동 시작. 재경기가 걸린 판은 아직 최종 결과가 아니므로 주문을 열지 않는다.
        if (!isRematch && ctx.triggerAutoOrder) ctx.triggerAutoOrder(gameState, room);
    ...
    function resetLadder(ld) {
        ...
        ld.winBottom = -1;            // server-only: 당첨 바닥칸
        ld.laneToBottom = [];
        ld.winLane = -1;              // server-only: 당첨 레인
        ld.userLanes = {};
        ld.participants = [];
        ld.revealOrder = [];
        ld.winners = [];
        ld.revealStartAt = 0;         // server-only: 재진입 seek 원점 폐기
        ld.revealPayload = null;      // server-only: 재진입 재전송 payload 폐기
        ld.isLadderActive = false;
    }

    // 게임 시작 — 스크램블 후 즉시 공개. 방장 [시작]과 예약 발화(재경기 자동 시작)가 같은 경로를 쓴다.
    // 호스트 검증은 호출부(소켓 핸들러)에서 한다 — 예약 발화에는 누른 사람이 없다.
    function startLadder(room, gameState, opts) {
        const ld = gameState.ladder;
        const reason = canStartLadder(room, gameState, opts);
        ...
        // 참가자 = 현재 방에 있고 준비한 사용자. 인원 상한 없음 — 중복 선택을 허용하므로 5명을 넘어도 된다.
        const participants = (gameState.readyUsers || []).filter(name =>
            gameState.users.some(u => u.name === name));
        const N = LADDER_LANES;   // 레인 항상 5 고정 — 아무도 안 고른 번호는 빈 레인으로 남는다
        ...
        // 시작 시점 유저 번호 확정: 참가자 소유 + 범위(0 ≤ lane ≤ N-1) 내인 것만 유지 (빌드 단계에서 고른 값)
        Object.keys(ld.userLanes).forEach(name => {
            const lane = ld.userLanes[name];
        ...
        ld.phase = 'selecting';            // 전이용 — 곧바로 doReveal
        ld.numLanes = N;
        ...
        // winBottom/winLane은 doReveal에서 "사람이 고른 레인 중"으로 확정(빈 레인 제외). 여기선 미설정.
        // ld.userLanes 유지 — 빌드 단계에서 고른 출발 번호. 미선택자는 doReveal에서 RNG 자동 배정.
        ld.participants = participants;
        ld.winners = [];
        ld.isLadderActive = true;

        console.log(`[사다리타기] 방 ${room.roomName} 시작 - 참가자 ${participants.length}명 / 레인 ${N}개${opts && opts.scheduled ? ' (예약 발화)' : ''}, 곧바로 공개`);
        doReveal(room, gameState);         // 별도 선택 단계 없이 즉시 공개
        updateRoomsList();
        return { ok: true };
    }

    return {
        clearLadderTimers, ladderLaneCount, readyCount, assignColorIndex, claimFreeLane,
        ensureBaseRungs, trimLadderBuildToN, emitRungsUpdated, emitLadderStateSync,
        doReveal, endGame, resetLadder, startLadder
    };
}

module.exports = (socket, io, ctx) => {
    const { updateRoomsList, getCurrentRoom, getCurrentRoomGameState } = ctx;
    // horse.js와 동일 패턴 — ctx가 주면 그걸 쓰고, 없으면 통과. (security-guard 훅이 ctx.checkRateLimit() 리터럴을 요구)
    const checkRateLimit = ctx.checkRateLimit ? () => ctx.checkRateLimit() : () => true;
    const engine = createLadderEngine(io, ctx);
    // 엔진 멤버는 전부 꺼내 쓴다 — 일부만 구조분해하면 빠뜨린 이름이 핸들러 실행 시점에 ReferenceError로
    // 터지고, 소켓 핸들러의 예외는 프로세스를 죽인다(핸들러가 "조용히 아무것도 안 하는" 증상으로 보인다).
    const {
        clearLadderTimers, ladderLaneCount, readyCount, assignColorIndex, claimFreeLane,
        ensureBaseRungs, trimLadderBuildToN, emitRungsUpdated, emitLadderStateSync,
        doReveal, endGame, resetLadder, startLadder
    ...
    socket.on('ladder:start', () => {
        ...
        // 호스트 검증은 여기서만 — 예약 발화(재경기 자동 시작)에는 누른 사람이 없다.
        const user = gameState.users.find(u => u.id === socket.id);
        if (!user || !user.isHost) {
        ...
        // 방장이 직접 시작하면 걸려 있던 재경기 예약은 해제한다(경마와 동일 — 앞지르기 허용).
        if (gameState.scheduledStartAt) {
            gameState.scheduledStartAt = null;
            require('./scheduled-start').broadcastSchedule(io, room, gameState);
        }

        const res = startLadder(room, gameState, {});
        if (!res.ok) socket.emit('ladder:error', res.error);
    });

    // 출발 번호 선택 (준비자, 빌드 단계) — 중복 허용, 재선택은 이동, 같은 번호 재클릭은 취소
    socket.on('ladder:pickLane', (data) => {
        ...
        // 중복 선택 허용 — 남이 이미 고른 번호도 고를 수 있다(경마에서 같은 말에 여럿 거는 것과 같다).
        // 같은 레인에 걸린 사람들은 한 토큰을 공유하고, 그 레인이 당첨되면 다 함께 당첨된 뒤 재경기로 갈린다.
        // 본인이 같은 번호를 다시 누르면 취소(미선택 → 시작 시 자동 배정).
        if (ld.userLanes[name] === lane) {
            delete ld.userLanes[name];
        } else {
            assignColorIndex(ld, name);   // 첫 등장이면 drawer 색 배정 (막대기 없이 번호만 골라도 색 부여)
            ld.userLanes[name] = lane;
        }
        emitRungsUpdated(room, gameState);
    });
...
// canStart는 순수 판정(거절 사유 문자열 또는 null). start는 엔진을 만들어 실행한다.
// 사다리는 중복 당첨 재경기를 이 경로로 자동 시작한다.
function canStartLadder(room, gameState) {
    if (!room || room.gameType !== 'ladder') return '사다리타기 방이 아니에요.';
    const ld = gameState && gameState.ladder;
    if (!ld) return '사다리 상태가 없어요.';
    if (ld.phase === 'finished') return '결과 정리 중이에요. 잠시 후 다음 판을 시작해주세요.';
    // idle(빌드)에서만 시작. finished 직후엔 곧 roundReset(resetTimeout)이 phase를 idle로 되돌리며
    // base/번호/색/막대기를 새로 초기화한다. finished에서 바로 시작하면 clearLadderTimers가 그 resetTimeout을
    // 취소해 resetLadder가 영영 안 돌고 이전 라운드 막대기/색/출발번호가 그대로 carry-over된다(빌드 desync).
    if (ld.phase !== 'idle') return '이미 게임이 진행 중이에요.';
    const ready = (gameState.readyUsers || []).filter(name =>
        gameState.users.some(u => u.name === name));
    if (ready.length < LADDER_MIN_PLAYERS) return `준비한 인원이 ${LADDER_MIN_PLAYERS}명 이상이어야 해요.`;
    return null;
}

module.exports.canStart = canStartLadder;
module.exports.start = (room, gameState, io, ctx, opts) =>
    createLadderEngine(io, ctx).startLadder(room, gameState, opts || {});
