...
const SCUFFLE_WAIT_MS = 80;       // 뚜껑 위에 이만큼 연속으로 머문 뒤에야 후보(착지 튕김 제외) — 만나면 바로 붙게(사용자 2026-09-21)
const SCUFFLE_SPEED = 60;         // 이 속도(px/s) 아래여야 "멈춰 기다리는 중"
const SCUFFLE_MIN_LEFT_MS = 400;  // 뚜껑이 이만큼은 더 닫혀 있어야 시작(열리기 직전에 붙어서 한 프레임 밀다 마는 깜빡임 방지)
const SCUFFLE_GRACE_MS = 300;     // 밀려나거나 속도가 튀어도 이 안에 돌아오면 계속(뚜껑이 열리면 즉시 끝). 80/60/400 조합에선 300 미만이면 공 풀기만 하다 마는 300ms 미만 몸싸움이 40시드에 67개 생긴다
