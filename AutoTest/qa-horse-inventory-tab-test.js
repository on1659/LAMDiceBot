...
// catalog 검증 아이템 (paint 슬롯에 코인 paint_gold + 광고 paint_ad_aqua 둘 다 존재 → 같은 슬롯 경쟁 테스트)
const COIN_PAINT = 'paint_gold';     // price 50, 비-광고
...
(async () => {
    ...
    //   코인 paint_gold(황금 도색)을 코인샵에서 구매+장착. 같은 paint 슬롯에 코인+광고 둘 다 장착 상태가 된다.
    //   빠른방(free)은 코인샵이 게이팅돼 카드가 안 나오므로, 코인 경제(서버 권위)는 그대로 두고
    //   클라 currentServerId만 정규값으로 올려 코인샵 카드 UI만 활성화한다(순수 UI 게이트 해제).
    await page.evaluate(() => { window.currentServerId = '__qa_unlock__'; });
    await gotoMain('코인샵');
    await gotoSub('도색');
    const buyCoinPaint = await page.evaluate(() => {
        const cards = Array.from(document.querySelectorAll('#horseShopMount .hshop-card'));
        const card = cards.find(c => c.querySelector('.hshop-name') && c.querySelector('.hshop-name').textContent === '황금 도색');
        const btn = card && card.querySelector('button.hshop-buy');
        if (!btn || btn.disabled) return { ok: false, text: btn ? btn.textContent : 'no-btn' };
        btn.click(); return { ok: true };
    });
    check(buyCoinPaint.ok, '[F3-5] 코인 paint(황금 도색) 구매 버튼 클릭: ' + JSON.stringify(buyCoinPaint));
    ...
    // ── [F3-6] 광고 paint 해제 → 코인 paint(황금 도색)이 ✓로 자동 복귀 ──
    ...
    check(afterUnequip && afterUnequip.winner.length === 1 && afterUnequip.winner.indexOf('황금 도색') >= 0,
        '[F3-6] 코인 paint(황금 도색) ✓ 자동 복귀(syncEquipBadge): ' + JSON.stringify(afterUnequip && afterUnequip.winner));

    // ── [F3-7] doEquip(코인) 후에도 인벤토리 메인탭 유지 ──
    //   인벤토리 카드의 코인 장착 버튼을 눌러도 inventory에 머무는지 (doEquip → renderModal)
    await page.evaluate(() => {
        // 황금 도색(현재 장착중) 클릭 → 해제. doEquip 경유.
        const cards = Array.from(document.querySelectorAll('#horseShopMount .hshop-inv-section .hshop-card'));
        const card = cards.find(c => c.querySelector('.hshop-name') && c.querySelector('.hshop-name').textContent === '황금 도색');
        const btn = card && card.querySelector('button.hshop-equip');
        if (btn) btn.click();
