...
description: ".Codex/ 하네스 폴더를 6개 룰로 점검 (worktree/lock/orphan/reference/hook/stale-agent). 보고서 → 사용자 승인 → 일괄 처리."
---

# source-command-harness-audit

Use this skill when the user asks to run the migrated source command `harness-audit`.

## Command Template

# Harness Audit — 하네스 점검

LAMDiceBot의 이더(Ether) 하네스(`.Codex/`)를 6개 룰로 점검한다. **메모리 폴더는 건드리지 않는다 — `/memory-audit`이 그 역할.**
...
```bash
ls "$CLAUDE_PROJECT_DIR/.Codex/"
git worktree list
```

검사 대상: `.Codex/agents/`, `.Codex/rules/`, `.Codex/skills/`, `.Codex/commands/`, `.Codex/hooks/`, `.Codex/worktrees/`, `.Codex/settings.json`, `.Codex/*.lock`.
**제외**: `.Codex/worktrees/` 하위는 worktree 자체가 검사 대상이지 내부 파일은 아님.
...
# 2) 외부 worktree(.Codex/worktrees/) 각각 dirty 검사
for wt in "$CLAUDE_PROJECT_DIR/.Codex/worktrees/"*/; do
...
```bash
for lock in "$CLAUDE_PROJECT_DIR/.Codex/"*.lock; do
    [ ! -f "$lock" ] && continue
...
```bash
for f in "$CLAUDE_PROJECT_DIR/.Codex/agents/"*.md \
         "$CLAUDE_PROJECT_DIR/.Codex/rules/"*.md \
         "$CLAUDE_PROJECT_DIR/.Codex/skills/"*.md; do
    [ ! -f "$f" ] && continue
    name=$(basename "$f" .md)
    refs=$(grep -rln "$name" --include="*.md" \
        "$CLAUDE_PROJECT_DIR/.Codex/" \
        "$CLAUDE_PROJECT_DIR/docs/" \
        "$CLAUDE_PROJECT_DIR/AGENTS.md" 2>/dev/null \
        | grep -v "^$f$" | wc -l)
...
grep -hoE '\]\([^)]+\.md\)' \
    ...
    "$CLAUDE_PROJECT_DIR/.Codex/rules/"*.md \
    "$CLAUDE_PROJECT_DIR/.Codex/agents/"*.md 2>/dev/null \
    | sed 's/.*(\(.*\))/\1/' \
    | sort -u \
    | while read p; do
...
grep -oE '\.Codex/hooks/[a-zA-Z0-9_-]+\.(sh|py)' \
    "$CLAUDE_PROJECT_DIR/.Codex/settings.json" \
    | sort -u \
    | while read h; do
...
```bash
find "$CLAUDE_PROJECT_DIR/.Codex/agents" -name "*.md" -mtime +60 \
    -exec stat -c "%y %n" {} \;
```
...
| # | 항목 | 룰 | 권장 액션 |
|---|------|-----|-----------|
| 1 | .Codex/worktrees/agent-xxx | 1 (clean) | git worktree remove |
| 2 | .Codex/worktrees/agent-yyy | 1 (dirty) | 사용자 결정 |
| 3 | .Codex/scheduled_tasks.lock | 2 | 즉시 삭제 |
| 4 | .Codex/agents/old.md | 6 | 사용자 검토 (90일+ 안 변경) |
```
...
- **메모리 폴더 안 건드림**: `~/.Codex/projects/.../memory/`는 `/memory-audit` 영역
- **`.Codex/worktrees/` 외부 파일 안 건드림**: worktree 자체만 검사 대상
...
- 짝꿍: `.Codex/commands/memory-audit.md`
- 하네스 룰 본체: `AGENTS.md` (구 `rules/harness.md`·`rules/workflow.md`는 2026-08-19 `docs/harness/archive/rules/`로 아카이브)
