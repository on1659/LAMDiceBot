---
name: "source-command-memory-audit"
description: "메모리 폴더 다이어트 진단. 6개 룰 적용해서 삭제/축소 후보를 보고서로 제시 후 사용자 승인 시 일괄 처리."
---

# source-command-memory-audit

Use this skill when the user asks to run the migrated source command `memory-audit`.

## Command Template

# Memory Audit — 메모리 다이어트 진단

`C:\Users\user\.Codex\projects\d--Work-LAMDiceBot\memory\` 폴더의 개별 메모리 파일을 6개 룰로 검사하여 다이어트 후보를 제시한다. **MEMORY.md(인덱스)는 검사 대상이 아니다 — 결과 반영 시에만 갱신.**
...
```bash
ls -la "$HOME/.Codex/projects/d--Work-LAMDiceBot/memory/"
```
...
`.Codex/rules/*.md`와 본문 비교. 70%+ 텍스트 유사도면 중복 플래그.
...
- 원칙: `~/.Codex/AGENTS.md` "auto memory" 섹션의 "What NOT to save"
