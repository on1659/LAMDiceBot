...
# source-command-summit

Use this skill when the user asks to run the migrated source command `summit`.
...
git log --oneline --encoding=utf-8 -10
```
...
### 작성 규칙:
- **유저가 볼 수 있는 내용만** 작성
- 기믹이나 확률 관련 된 내용은 작성 금지
- 신규 추가되는 기능에 대해서 100% 보여지는 행동이 아닌이상 금지 (10%확률로 등장하는 기믹등)
- 내부 로직, 서버 구현 상세, DB 스키마 변경 등 민감한 정보 절대 금지
- UI 변경, 새 기능, 버그 수정 등 체감 가능한 변경만 기재
- 기존 형식(HTML 태그 + 이모지) 유지
- 업데이트 날짜가 같으면 같은 업데이트항목란에 추가하지 않고 기존 항목에 추가
