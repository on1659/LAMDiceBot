...
**실시간 멀티플레이어 게임 플랫폼 - 주사위, 룰렛, 경마, 팀 배정 게임을 한 곳에서!**

## ✨ 주요 특징

- 🎯 **완전한 공정성**: 서버에서 난수 생성 (조작 불가능)
- 🌐 **실시간 멀티플레이어**: WebSocket 기반 실시간 동기화
- 🎲 **다양한 게임 모드**: 주사위, 룰렛, 경마, 팀 배정 게임
...
### 🐎 경마 게임
- 실시간 말 달리기 애니메이션
- 서버에서 속도/순위 계산으로 공정한 결과
- 트랙 배경·말 스프라이트 리소스 지원
- 멀티플레이어 동시 관전
...
### v2.4 (2026-01)
- **경마 게임 모드**: 실시간 경마 멀티플레이어 지원
- **마지막 사람 당첨 버그 수정**: 룰렛 등 결과 계산 오류 해결
- **테스트 구조 정리**: 다이스/룰렛/경마 테스트 → `AutoTest/dice`, `AutoTest/roulette`, `AutoTest/horse-race`
- **공통 설정**: `config.js` + `.env` 기반 PORT/BASE_URL (서버·테스트·봇 공용)

### v2.3 (2026-01)
- **룰렛 이름 표시 스타일 개선**: 하얀 테두리 효과 완화
- **팀 배정·룰렛 게임**: 주문받기 기능 연동

### v2.2 (2025-12-21)
- **호스트 강퇴 기능**: 더블 클릭으로 불성실 참여자 제외
- **자동 게임 종료**: 중도 퇴장 시 즉시 결과 도출
...
**자세한 배포 가이드:** `docs/Railway배포완벽가이드.md` 참조

## 🎮 사용 방법

### 기본 플레이
1. **Host 입장**: 한 명이 "Host로 입장" 체크하고 입장
2. **게임 타입 선택**: 주사위/룰렛/경마/팀 배정 중 선택
...
### 경마 게임
1. **준비하기**: 참가자들이 "준비" 버튼 클릭 (최소 인원 준비)
2. **경마 시작**: Host가 경마 시작 버튼 클릭
3. **결과 확인**: 말 달리기 애니메이션 후 순위·결과 표시
...
```
LAMDiceBot/
├── server.js                         # 서버 (난수 생성, WebSocket)
├── config.js                         # 공통 설정 (.env 기반 PORT/BASE_URL)
├── dice-game-multiplayer.html        # 주사위 게임
├── roulette-game-multiplayer.html    # 룰렛 게임
├── horse-race-multiplayer.html       # 경마 게임
├── team-game-multiplayer.html        # 팀 배정 게임
├── frequentMenus.json                # 자주 쓰는 메뉴 목록
├── suggestions.json                 # 게시판/제안 데이터
├── package.json                     # 의존성
├── .env                             # 환경 변수 (PORT, BASE_URL 등)
├── README.md                        # 이 파일
├── CHANGELOG.md                     # 변경 내역
├── AutoTest/                        # 자동/봇 테스트
│   ├── dice/                        # 주사위 테스트 봇
...
└── docs/
    ...
    │   ├── dice/                    # 주사위 문서·계획
    │   └── horse/                   # 경마 문서·리소스
    └── system/                      # 공통 모듈·QA·사운드 등
```

## 🛠️ 설정 변경

### 포트·BASE_URL (권장)
`.env` 파일에서 설정 (서버·테스트·봇 공용):
```
PORT=3000
BASE_URL=http://localhost:3000
```
`config.js`가 `.env`를 읽어 `PORT`, `BASE_URL`을 내보냅니다.

### 포트만 코드에서 변경
`server.js`에서 `config.js`의 `PORT` 사용 또는:
```javascript
...
**버전:** 2.4 (경마 게임, 테스트 구조 정리, config.js)  
**최종 업데이트:** 2026-01  
**보안 등급:** ⭐⭐⭐⭐⭐
