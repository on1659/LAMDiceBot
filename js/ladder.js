...
// ── 게임 상태 변수 (서버 권위 — ladder:rungsUpdated / ladder:reveal 로만 갱신) ──
// 사다리 구조·번호·당첨은 전부 서버가 정한다. 클라는 받은 값을 그리기만 한다(재계산 금지 — 공정성).
...
function ladderEnterRoom(data, asHost) {
    ...
    // 빌드 화면 즉시 렌더 — 첫 ladder:rungsUpdated 도착 전에도 번호 그리드/캔버스가 보이게.
...
socket.on('updateUsers', (data) => {
    ...
    // isHost가 여기서 바뀔 수 있으므로 시작 버튼과 번호 고르기 그리드를 재적용
    updateStartButton();
    renderBuildSection();
});
...
var LADDER_BOMB_POINTER_MS = 5200;  // 폭탄 룰렛 포인터가 바닥칸을 가속→감속하며 훑다 winBottom(당첨 칸)에 정지(하강 전) (2600×2) — socket/ladder.js와 동일 유지
...
var ladderState = {
    ...
    winBottom: -1,      // 당첨 바닥칸(서버 권위)
    winLane: -1,        // 당첨 레인(서버 권위)
    laneToBottom: [],
    userLanes: {},      // {name: lane} — 중복 허용(여러 이름이 같은 값)
    participants: [],
    winners: [],        // 당첨자 — winLane을 고른 사람 전원
    ...
    bombRevealed: false // 폭탄 포인터가 winBottom에 멈춰 🎯당첨 칸을 공개한 뒤 true(하강 전). 누가 걸렸는지는 하강 후.
};

// 현재 판의 토큰 경로 — 백그라운드 탭에서 gameEnd가 연출보다 먼저 올 때 최종 프레임을 그리는 데 쓴다.
var ladderRevealPaths = null;
...
// 준비하고 현재 방에 있는 사람 수 — 레인은 항상 5 고정이라 빌드 노출/시작 게이트는 이 값(≥2)으로 판단.
function readyCount() {
...
function renderBuildLaneGrid(N, ready) {
    ...
    // 번호별로 그 번호를 고른 사람 전원을 모은다 — 중복 선택을 허용하므로 한 번호에 여러 명이 붙는다.
    var byLane = {};
    Object.keys(buildState.userLanes).forEach(function (n) {
        var l = buildState.userLanes[n];
        (byLane[l] = byLane[l] || []).push(n);
    });

    for (var i = 0; i < N; i++) {
        var owners = byLane[i] || [];
        var mine = owners.indexOf(currentUser) >= 0;
        var btn = document.createElement('div');
        btn.className = 'ladder-lane-btn';
        // 남이 골랐다고 잠그지 않는다(경마에서 같은 말에 여럿 걸 수 있는 것과 같다).
        // 상태는 세 가지: 내가 고름 / 남만 고름 / 아무도 안 고름.
        if (mine) btn.classList.add('mine');
        else if (owners.length) btn.classList.add('shared');
        else btn.classList.add('empty');   // 주인 없는 빈 레인 — 이 번호는 당첨 후보에서 빠진다

        var ownerLabel;
        if (!owners.length) {
            ownerLabel = '<span class="lane-owner">비어있음</span>';
        } else {
            // 내 이름을 맨 앞으로 올려 한눈에 보이게 한다.
            var sorted = owners.slice().sort(function (x, y) {
                return (x === currentUser ? -1 : 0) - (y === currentUser ? -1 : 0);
            });
            var shown = sorted.slice(0, 2).map(function (n) {
                return escapeHtml(n) + (n === currentUser ? ' (나)' : '');
            }).join(', ');
            if (sorted.length > 2) shown += ' 외 ' + (sorted.length - 2) + '명';
            ownerLabel = '<span class="lane-owner">' + shown + '</span>';
        }
        ...
        if (ready) {
            ...
            btn.setAttribute('aria-label', (i + 1) + '번' + (mine ? ' 내 선택, 다시 누르면 취소' : ' 선택')
                + (owners.length ? ', 고른 사람 ' + owners.length + '명' : ''));
            (function (lane) {
                function pick() {
            ...
            // 준비 전에는 고를 수 없다 — 상태만 스크린리더에 전달, 포커스 비대상
            btn.setAttribute('aria-label', (i + 1) + '번, 준비하면 고를 수 있어요');
        }
...
// 바닥칸 라벨 그리기 — 「당첨」 칸은 폭탄 포인터가 착지한 "후"에만 보인다(빌드/대기 단계엔 비공개 — 누출 금지).
//   · 포인터 착지 완료 + 그 칸이 winBottom → 🎯당첨
//   · 폭탄 포인터가 지금 가리키는 칸(공개 전) → 강조 배경(룰렛 느낌)
//   · 그 외 칸 → 빈칸(라벨 없음). 당첨 칸이 먼저 공개되므로 긴장은 "누가 그 칸에 도착하나"에 있다.
...
function drawBottomSlots(ctx, W) {
    ...
    for (let i = 0; i < N; i++) {
        const x = laneX(W, i, N);
        const isBomb = ladderState.bombRevealed && i === ladderState.winBottom;
...
function markWinnerLaneName() {
    const cont = document.getElementById('ladderLaneNames');
    if (!cont) return;
    cont.querySelectorAll('.ladder-lane-name').forEach(el => el.classList.remove('active'));
    if (ladderState.winLane < 0) return;
    cont.querySelectorAll('.ladder-lane-name').forEach(el => {
        if (String(el.dataset.lane) === String(ladderState.winLane)) el.classList.add('winner');
    });
}
...
function startReveal(data) {
    ...
    ladderState.winBottom = data.winBottom;
    ladderState.winLane = (typeof data.winLane === 'number') ? data.winLane : -1;
    ladderState.laneToBottom = data.laneToBottom || [];
    ladderState.userLanes = data.userLanes || {};
    ladderState.winners = data.winners || [];
    // 바닥 공개/폭탄 상태 초기화 — 새 판은 바닥 라벨 전부 빈칸으로 시작, 폭탄 미공개.
    ladderState.bombPointerCol = -1;
    ladderState.bombRevealed = false;
    ladderPhase = 'revealing';   // 셸의 isLadderActive()/OrderModule 이 읽는 미러(함수를 덮어쓰지 말 것)
    ...
    // 토큰은 **레인당 하나**다 — 같은 번호를 고른 사람들은 한 토큰을 공유한다(중복 선택 허용).
    // 서버 revealOrder는 점유 레인 인덱스 배열(서버 RNG 셔플). 모든 탭이 같은 순서로 재생한다.
    const laneOwners = {};
    Object.keys(ladderState.userLanes).forEach(function (n) {
        const l = ladderState.userLanes[n];
        (laneOwners[l] = laneOwners[l] || []).push(n);
    });
    const order = (Array.isArray(data.revealOrder) && data.revealOrder.length)
        ? data.revealOrder.filter(lane => (laneOwners[lane] || []).length > 0)
        : Object.keys(laneOwners).map(Number);

    const paths = order.map(lane => {
        const pts = buildPath(lane);
        const owners = laneOwners[lane] || [];
        // 여러 명이면 "이름 외 N명"으로 줄여 라벨이 옆 레인을 침범하지 않게 한다.
        const label = owners.length > 1 ? (owners[0] + ' 외 ' + (owners.length - 1) + '명') : (owners[0] || '');
        return {
            lane: lane,
            owners: owners,
            name: (lane + 1) + '번 ' + label,
            startLane: lane,
            color: rungColor(owners[0]),   // drawer 색(colorIndex) — 빌드/막대기/토큰 색 일관
            pts: pts,
    ...
    ladderRevealPaths = paths;   // 백그라운드 탭 jumpToFinal용 보관

    const N = paths.length;
    if (N === 0) { drawScrambleStatic(1, 0); return; }

    // ── 재진입 seek ──
    // 서버가 진행 중 재진입자에게 reveal payload + elapsedMs를 개인 emit한다(emitLadderStateSync).
    // 처음부터 재생하면 다른 사람 화면보다 늦어 결과가 어긋나 보이므로, 지난 만큼 건너뛴다.
    const elapsed = Math.max(0, Number(data.elapsedMs) || 0);
    const preMs = ladderPreDescentMs();
    if (elapsed > 0) {
        const tp = new Array(N).fill(0);
        if (elapsed >= preMs + ladderDescentSlots(N) * LADDER_TOKEN_SLOT_MS) {
            // 하강까지 다 끝난 뒤 들어옴 → 최종 프레임
            ladderState.bombRevealed = true;
            for (let i = 0; i < N; i++) tp[i] = 1;
            drawLadderFrame(paths, tp);
            markWinnerLaneName();
            return;
        }
        if (elapsed >= preMs) {
            // 하강 중 — 지난 만큼 앞당겨 이어붙인다(당첨 칸은 이미 공개된 시점)
            revealBomb(paths, tp, elapsed - preMs);
            return;
        }
        // 아직 사전 연출 구간 — 스크램블 완료 화면을 정적으로 보여주다가 남은 시간 뒤 하강에 합류한다.
        // (지우기/그리기/포인터 애니를 중간부터 재생하는 것보다, 다른 사람과 하강 시점을 맞추는 게 중요하다.)
        drawScrambleStatic(1, 1);
        ladderRevealTimers.push(setTimeout(function () {
            revealBomb(paths, new Array(N).fill(0), 0);
        }, preMs - elapsed));
        return;
    }
...
// 하강 슬롯 수 — 서버 socket/ladder.js의 ladderDescentSlots와 **반드시 동일**.
// 마지막 두 토큰이 한 슬롯을 공유하므로 토큰 수보다 하나 적다(N>=2).
function ladderDescentSlots(N) {
    var n = Math.max(0, N | 0);
    return n <= 1 ? n : n - 1;
}
// 하강 시작 전까지의 연출 합(ms) — 카운트다운 → 지우기 → 그리기 → 바닥멈춤 → 폭탄 포인터.
// 재진입 seek의 기준점이다. 서버 ladderRevealDelay의 앞부분과 같은 항으로 구성해야 한다.
function ladderPreDescentMs() {
    return LADDER_COUNTDOWN_MS + LADDER_ERASE_MS + LADDER_DRAW_MS + LADDER_BOTTOM_PAUSE_MS + LADDER_BOMB_POINTER_MS;
}
...
function runDescentPhase(paths, seekMs) {
    const N = paths.length;
    // 토큰별 진행도 — 인덱스 순서 = paths(=서버 revealOrder). 0=대기, 1=도착.
    const tokenProgress = new Array(N).fill(0);

    // 하강 슬롯 = 서버 ladderDescentSlots와 동일해야 한다(어긋나면 서버 endGame이 연출 도중 끼어든다).
    // 마지막 두 토큰은 한 슬롯을 공유하며 **같이** 내려간다 — 마지막 한 명의 결과가 미리 확정돼
    // 김빠지는 것을 막는다. 그래서 슬롯 수는 토큰 수보다 하나 적다(N>=2일 때).
    // 각 스텝이 내보내는 토큰 묶음: [0], [1], ... , [N-3], [N-2, N-1]
    const steps = [];
    if (N === 1) steps.push([0]);
    else if (N >= 2) {
        for (let k = 0; k < N - 2; k++) steps.push([k]);
        steps.push([N - 2, N - 1]);
    }
    ...
    if (isLocalhost) {
        ...
        addDebugLog('순차 reveal N=' + N + ' 슬롯=' + steps.length + ' SLOT=' + LADDER_TOKEN_SLOT_MS + 'ms 총하강=' +
            (steps.length * LADDER_TOKEN_SLOT_MS) + 'ms 경로px=[' + lens.map(function (l) { return Math.round(l); }).join(', ') + ']');
    }

    // 한 스텝(토큰 1개 또는 마지막 쌍 2개)을 SLOT_MS 동안 0→1로 내려보낸다.
    // 끝나면 다음 스텝, 모두 끝나면 finishDescent().
    function animateStep(si, offsetMs) {
        const group = steps[si];
        const status = document.getElementById('gameStatus');
        if (status) {
            const nm = group.map(function (k) { return paths[k] && paths[k].name; }).filter(Boolean).join(' · ');
            status.textContent = group.length > 1
                ? ('🪜 마지막 두 줄이 함께 내려갑니다... ' + nm)
                : ('🪜 한 줄씩 사다리를 타고 내려갑니다... ' + nm + ' (' + (si + 1) + '/' + steps.length + ')');
            status.className = 'game-status active';
        }
        playLadderSound('ladder_descend', 0.6);   // 스텝마다 효과음

        const start = performance.now() - (offsetMs || 0);   // seek: 이미 지난 만큼 앞당겨 시작
        function frame(now) {
            const t = Math.min(1, (now - start) / LADDER_TOKEN_SLOT_MS);
            group.forEach(function (k) {
                // 중력 time-warp: 선형 시간 t → 호 길이 비율(하향 가속·상향 감속). 총시간은 SLOT_MS 그대로(t=1→1).
            ...
            if (t >= 1) {
                group.forEach(function (k) { tokenProgress[k] = 1; });   // 도착 — 그 자리에 계속 남음
                drawLadderFrame(paths, tokenProgress);
                ladderAnimRAF = null;
                if (si + 1 < steps.length) animateStep(si + 1);
                else finishDescent(paths, tokenProgress);
                return;
            }
    ...
    // 마지막 토큰 도착 후: 당첨 레인 토큰이 (이미 공개된) 🎯칸에 도착 → 그 레인 이름 강조 + 결과 캡션.
    // 당첨 칸은 하강 전 포인터에서 이미 공개됐으므로 여기선 "도착" 강조만 한다.
    function finishDescent(paths, tokenProgress) {
        drawLadderFrame(paths, tokenProgress);
        markWinnerLaneName();
        const names = (ladderState.winners || []).join(', ');
        const many = (ladderState.winners || []).length >= 2;
        const cap = document.getElementById('ladderResultCaption');
        if (cap) {
            cap.textContent = names
                ? (many ? `🎯 ${names} 님 동시 당첨! (${ladderState.winLane + 1}번)` : `🎯 ${names} 님 당첨! (${ladderState.winLane + 1}번)`)
                : '결과 집계 중...';
        }
        const status = document.getElementById('gameStatus');
        if (status) { status.textContent = names ? `🎯 ${names} 님 당첨!` : '게임 종료'; status.className = 'game-status active'; }
        playLadderSound('ladder_result', 1.0);
    }

    if (N === 0) { drawScrambleStatic(1, 0); return; }

    // 재진입 seek — 하강이 이미 seekMs 만큼 진행된 상태로 이어붙인다.
    // 지나간 스텝의 토큰은 도착(1)으로 채우고, 진행 중인 스텝은 그 지점부터 애니메이션한다.
    var startStep = 0, offsetMs = 0;
    if (seekMs > 0) {
        startStep = Math.floor(seekMs / LADDER_TOKEN_SLOT_MS);
        offsetMs = seekMs - startStep * LADDER_TOKEN_SLOT_MS;
        if (startStep >= steps.length) {   // 하강이 이미 끝난 시점에 들어옴 → 최종 프레임으로
            for (var k = 0; k < N; k++) tokenProgress[k] = 1;
            finishDescent(paths, tokenProgress);
            return;
        }
        for (var si = 0; si < startStep; si++) {
            steps[si].forEach(function (k) { tokenProgress[k] = 1; });
        }
    }

    // 첫 프레임에서 대기 점이 보이도록 한 번 그린 뒤 스텝 출발.
    drawLadderFrame(paths, tokenProgress);
    animateStep(startStep, offsetMs);
}

// 폭탄 룰렛 포인터 — 순수 연출, 하강 "전"에 실행. 바닥칸(0..N-1)을 가속→감속하며 훑다 서버값 winBottom에 정지.
//   포인터 위치는 클라가 시각화만; 착지칸은 항상 ladderState.winBottom(서버 권위). 클라 Math.random 0회.
//   총 이동 = (몇 바퀴) + (winBottom까지 거리) 칸을 ease-out(감속)으로 주파 → 끝 프레임에서 정확히 착지.
...
function runBombPointerPhase(paths, tokenProgress) {
    const N = ladderState.numLanes;
    const target = ladderState.winBottom;
...
// 폭탄 착지 후 칸 공개 — 「당첨」 칸만 보인다(bombRevealed=true). 누가 걸렸는지는 하강 후 공개.
//   바로 순차 하강을 시작해 "누가 이 칸에 도착하나"를 보여준다.
function revealBomb(paths, tokenProgress, seekMs) {
    ladderState.bombRevealed = true;
    ladderState.bombPointerCol = -1;
    drawLadderFrame(paths, tokenProgress);
    const cap = document.getElementById('ladderResultCaption');
    if (cap) cap.textContent = '🎯 당첨 칸이 정해졌어요 — 누가 도착할까요?';
    const status = document.getElementById('gameStatus');
    if (status) { status.textContent = '🎯 당첨 칸 확정! 누가 도착할까요...'; status.className = 'game-status active'; }
    playLadderSound('ladder_result', 0.7);
    runDescentPhase(paths, seekMs || 0);   // 당첨 칸 공개 후 순차 하강 시작
}

// 백그라운드 탭 대응 — 서버 gameEnd가 연출보다 먼저 도착하면 남은 프레임을 건너뛰고 최종 상태로 맞춘다.
// RAF는 탭이 숨겨지면 멈추므로, 이게 없으면 돌아왔을 때 토큰이 중간에 얼어 있고 결과와 어긋난다. 멱등.
function ladderJumpToFinal() {
    if (!ladderRevealPaths || !ladderRevealPaths.length) return;
    if (ladderAnimRAF) { cancelAnimationFrame(ladderAnimRAF); ladderAnimRAF = null; }
    clearLadderRevealTimers();
    ladderState.bombRevealed = true;
    ladderState.bombPointerCol = -1;
    const done = new Array(ladderRevealPaths.length).fill(1);
    drawLadderFrame(ladderRevealPaths, done);
    markWinnerLaneName();
}
...
socket.on('ladder:reveal', (data) => {
    ...
    startReveal(data);
    addDebugLog('공개: 당첨=' + ((data.winners || []).join('/') || '없음') + ' (' + ((data.winLane|0) + 1) + '번)');
});

socket.on('ladder:gameEnd', (data) => {
    ladderState.phase = 'finished';
    document.body.classList.remove('race-running');   // 연출 종료(결과) — 스티키 광고 복원(C-6)
    ladderPhase = (ladderState.phase === 'finished') ? 'finished' : 'idle';   // 셸 미러 갱신(함수 덮어쓰기 금지)
    // 백그라운드 탭이면 RAF가 멈춰 연출이 중간에 얼어 있다 — 결과와 어긋나지 않게 최종 프레임으로 맞춘다.
    ladderState.winners = Array.isArray(data.winners) ? data.winners : [];
    if (typeof data.winLane === 'number') ladderState.winLane = data.winLane;
    if (typeof data.winBottom === 'number') ladderState.winBottom = data.winBottom;
    ladderJumpToFinal();
    ...
    // 결과 오버레이 — 당첨자를 맨 위로 올린다("이 사람이 걸렸다"가 먼저 읽히게)
    const rankingsEl = document.getElementById('resultRankings');
    if (rankingsEl && Array.isArray(data.rankings)) {
        const ordered = data.rankings.slice().sort((a, b) => (b.isWinner ? 1 : 0) - (a.isWinner ? 1 : 0));
        rankingsEl.innerHTML = ordered.map(r => {
            const tag = r.isWinner
                ? `<span class="ladder-result-tag winner">🎯 당첨</span>`
                : `<span class="ladder-result-tag pass">— 비켜감</span>`;
            return `<div class="ladder-result-row${r.isWinner ? ' winner' : ''}">
                <span class="ladder-result-name">${escapeHtml(r.name)} <span class="ladder-result-lane">(${r.lane + 1}번)</span></span>${tag}</div>`;
        }).join('');
    }
    // 동시 당첨이면 다음 판 안내를 결과창에 붙인다(채팅 안내와 짝 — 결과창만 보고 있어도 알 수 있게).
    const noteEl = document.getElementById('resultNote');
    if (noteEl) {
        noteEl.textContent = rematch
            ? `${winners.join(', ')} 님이 ${(data.winLane + 1)}번에서 동시 당첨! 두 분끼리 곧 한 판 더 시작해요.`
            : '';
    ...
    const status = document.getElementById('gameStatus');
    if (status) {
        status.textContent = winners.length
            ? (rematch ? `🎯 ${winners.join(', ')} 님 동시 당첨 — 한 판 더!` : `🎯 ${winners[0]} 님 당첨!`)
            : '게임 종료';
...
socket.on('ladder:roundReset', () => {
    ...
    document.body.classList.remove('race-running');   // 잔존 시 스티키 광고 영구 숨김(C-6)
...
// 재진입 동기화 — 서버가 개인 emit으로 방 기록을 되돌려준다(마스킹 때문에 roomJoined엔 없다).
// 진행/결과 중이면 뒤이어 ladder:reveal(+elapsedMs)와 ladder:gameEnd가 따로 온다.
socket.on('ladder:stateSync', (d) => {
...
socket.on('ladder:gameAborted', (data) => {
    document.body.classList.remove('race-running');   // 중단 시에도 스티키 복원(C-6)
    ladderState.phase = 'idle';
