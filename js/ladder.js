...
var LADDER_CURVE_SMOOTH_STEPS = 8;  // 곡선 렌더 시 점 사이를 Catmull-Rom으로 채울 분할 수(시각만 — 데이터/결과 무관)
// 곡선이 두 기둥 사이 칸 밖으로 삐져나올 수 있는 마진(가운데 점만 — 양끝은 항상 기둥에 스냅).
// x: 칸 너비 대비, y: 전체 높이 대비. 캔버스 패딩 안에 들어오는 값(빌드 pad40 / 리빌 pad56)이라 토큰도 화면 안에 머묾.
// 서버 socket/ladder.js의 같은 상수와 반드시 동일 값 유지(한 배포로 묶음). points는 시각만 — 결과 매핑 무관.
var LADDER_CURVE_MARGIN_X = 0.35;
var LADDER_CURVE_MARGIN_Y = 0.1;
...
// 좌표 clamp(칸 밖 마진까지 허용) + 개수 상한 + 양끝을 두 기둥(x=0,1)에 스냅.
...
function clampCurveVTravel(pts) {
    ...
    return pts.map(function (p) { return { x: p.x, y: clampCurveY(meanY + (p.y - meanY) * k) }; });
}
...
// 균등 Catmull-Rom 스플라인 — 원본 점을 모두 지나면서 그 사이를 부드럽게 보간한다(시각만).
//   pts 양끝은 그대로 보존(기둥 스냅 좌표 유지). 점이 적어도 곡선으로 보이게 점 사이를 steps만큼 채운다.
...
function smoothPolyline(pts, steps) {
    if (!Array.isArray(pts) || pts.length < 3 || steps < 2) return pts;
    var out = [pts[0]];
    for (var i = 0; i < pts.length - 1; i++) {
        ...
        for (var s = 1; s <= steps; s++) out.push(catmullRomPoint(p0, p1, p2, p3, s / steps));
    }
    return out;
}
...
//   곡선 points는 Catmull-Rom으로 보간해 점이 적어도(빠르게 긋거나 다운샘플돼도) 부드러운 곡선으로 그린다.
function rungToPolyline(rg, xOf, yOf, halfOf) {
    ...
    if (rg.points && rg.points.length >= 2) {
        ...
        return smoothPolyline(out, LADDER_CURVE_SMOOTH_STEPS);
    }
    var yc = yOf(rg.y);
...
function buildPxToY(py) { return clampCurveY((py - BUILD_TOP) / (BUILD_H - 2 * BUILD_TOP)); }
...
//   points = 궤적을 셀 좌표로 정규화(가운데는 칸 밖 마진까지 허용) + 양끝을 두 기둥에 스냅 + 다운샘플(시각, 결과 무관).
...
function computeDragRung(N) {
    ...
    for (var k = 0; k < seq.length; k++) {
        pts.push({
            x: clampCurveX(span > 0 ? (seq[k].x - xL) / span : 0),
